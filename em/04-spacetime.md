...
[[7shi-cla1]]の$\operatorname{Cl}_{n,0}(\mathbb R)$では、作用素の係数に生成元をそのまま使いました。生成元の2乗が$-1$のものがある場合は、逆元$\gamma^\mu=\gamma_\mu^{-1}$を係数にします。
...
逆元を使うのは、$\gamma^\mu\gamma_\mu=1$によって$Dx=\sum_\mu\gamma^\mu\gamma_\mu=4$となり、[[7shi-cla5]]の$D\boldsymbol x=n$と同じ規約が保たれるためです。$D$の2乗は、[[7shi-cla1]]と同じ計算で係数の2乗の和になります。
...
[[7shi-cla4]]では、$\operatorname{Cl}_{4,0}(\mathbb R)$で位置ベクトルに$e_0$を掛けて四元数の変数$q=e_0\boldsymbol x$を作り、フューター作用素を$\mathcal D=e_0D=\partial_0+\sum_lh_l\partial_l$（$h_l=e_0e_l$）としました。上の[作用素の分割](#fml-split)は、同じ操作を$\operatorname{Cl}_{1,3}(\mathbb R)$で行ったものです。
...
| | 代数 | 空間の生成元の2乗 | 偶部分の生成元 | その2乗 | 共役との積 | 型 |
|---|---|---|---|---|---|---|
...
どちらも「2乗が$+1$の時間方向の生成元を掛けて偶部分に移す」操作で、得られる作用素は$\partial_0+\sum(\text{偶部分の生成元})\partial$という同じ形をしています。違いは空間の生成元の2乗の符号だけで、それが偶部分の生成元の2乗の符号を反転させ、ラプラシアンと波動作用素を分けます。$\operatorname{Cl}_{n,0}(\mathbb R)$の枠組みでは空間の生成元の2乗は$+1$に固定されていたので、双曲型の作用素を得るには符号数そのものを替える必要がありました。

# マクスウェル方程式

## 場と電流

$\operatorname{Cl}_{3,0}(\mathbb R)$の$F=\boldsymbol E+Ic\boldsymbol B$を偶部分代数の元として読み直します。
...
であり、[[7shi-em2]]のスカラー部とベクトル部が$D\cdot F$から、2ベクトル部と擬スカラー部が$D\wedge F$から来ています。
...
&&&prf
2ベクトル$F$について$D\cdot(D\cdot F)=\sum_{\mu,\nu}\gamma^\mu\cdot(\gamma^\nu\cdot F)\,\partial_\mu\partial_\nu$である。$\gamma^\mu\cdot(\gamma^\nu\cdot F)$はスカラーで、$F=\gamma_\alpha\wedge\gamma_\beta$の形の基底について$\mu,\nu$を入れ替えると符号が変わる。偏微分は交換するので、和は$0$になる。
&&&
...
- **係数の2乗の符号**：$\gamma_0$を掛ける操作は[[7shi-cla4]]の$e_0$を掛ける操作と同じで、空間の生成元の2乗の符号が偶部分の生成元の2乗の符号を反転させ、楕円型と双曲型を分けます。
