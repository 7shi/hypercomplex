...
日常の感覚では、速さは観測者によって変わります。静止した観測者に対して$x_1$の向きに速さ$v$で動く観測者の座標は$x_1'=x_1-vt$、$t'=t$で与えられると考えるのが自然です（ガリレイ変換）。この変換では、静止した観測者から見て速さ$c$で進む光は、動く観測者からは速さ$c-v$で進んで見えます。そうするとマクスウェル方程式は特別な1つの観測者に対してだけ成り立つことになり、19世紀には光を伝える媒質（エーテル）が静止している観測者がそれだと考えられました。しかし、地球の運動による光の速さの違いを検出しようとした実験（マイケルソン＝モーリーの実験、1887年）では、違いは見つかりませんでした。

アインシュタインは1905年に、次の2つを原理として置きました。

- **相対性原理**：互いに一定の速度で動く観測者（慣性系）の間で、物理法則は同じ形を取ります。
...
です。光速度不変の原理から、この式はどの慣性系でも成り立ちます。さらに一般に、2つの事象の差について$c^2\Delta t^2-|\Delta\boldsymbol x|^2$（間隔の2乗）はすべての慣性系で同じ値を取ります。ローレンツ変換とは、この量を保つ座標の変換です。本記事では、この量を時空のベクトルの2乗として表す代数を使います。
...
- **因果**：物体も信号も光より速くは進めないので、ある事象が影響を及ぼせるのは、差が時間的または光的で未来向きの事象だけです。差が空間的な2つの事象の順序は観測者によって入れ替わり得ますが、互いに影響し合うことはないので矛盾は生じません。
- **世界線と観測者**：粒子が時空に描く曲線を世界線と呼びます。粒子の速さは$c$より小さいので、世界線の接ベクトルは時間的です。観測者は、自分の世界線の向きを時間方向とし、それに直交する方向を空間とします。本記事の$\gamma_0$は、静止した観測者の時間方向です。
...
$J$を**4元電流**とも呼びます。電荷密度と電流密度が1つのベクトルの成分になっているので、時間方向の取り方が替われば両者も混ざります。静止した電荷の分布は、それに対して動く観測者からは電荷の流れ、すなわち電流を伴って見えます。電場と磁場の場合も同じく、分け方は観測者の時間方向に依存します。
