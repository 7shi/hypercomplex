...
アインシュタインは1905年に、次の2つを原理として置きました。

- **相対性原理**：すべての慣性系で、物理法則は同じ形を取ります。
- **光速度不変の原理**：真空中の光の速さは、どの慣性系で測っても同じ$c$です。

2つを両立させるにはガリレイ変換を捨て、時間と空間の座標が互いに混ざる変換を受け入れる必要があります。それが**ローレンツ変換**です。とくに、離れた2点で「同時」かどうかは観測者によって変わります。マクスウェル方程式は、もともとローレンツ変換に対して形を保つ方程式でした。

&&&ex 同時性の相対性
ある慣性系に静止した観測者に対して$x_1$の向きに速さ$v$で動く観測者の座標は、ローレンツ変換

$$
x_0'=\gamma(x_0-\beta x_1),\qquad x_1'=\gamma(x_1-\beta x_0),\qquad x_2'=x_2,\qquad x_3'=x_3,\qquad
...
時刻と位置の組$(t,\boldsymbol x)$で指定される時空の点を**事象**と呼びます。事象$(t_1,\boldsymbol x_1)$で出た光が事象$(t_2,\boldsymbol x_2)$に届くなら$|\boldsymbol x_2-\boldsymbol x_1|=c(t_2-t_1)$、すなわち

$$
c^2(t_2-t_1)^2-|\boldsymbol x_2-\boldsymbol x_1|^2=0
$$

です。光速度不変の原理から、この式はどの慣性系でも成り立ちます。2つの事象の差について$c^2\Delta t^2-|\Delta\boldsymbol x|^2$を**間隔の2乗**と呼びます。光速度不変が直接求めるのは、この量が$0$であるという関係が保たれることです。相対性原理と時空の一様性・空間の等方性を合わせて慣性系の間の変換を求めると、$0$でない場合も含めて、この量そのものがすべての慣性系で同じ値を取ることが分かります。本記事ではその導出は省き、結果を使います。ローレンツ変換とは、この量を保つ線形の座標変換です。本記事では、この量を時空のベクトルの2乗として表す代数を使います。
...
と書くと、$x^2=x_0^2-x_1^2-x_2^2-x_3^2$です。ベクトルの2乗が符号数$(+,-,-,-)$のミンコフスキー計量を与え、2つの事象の差$x$の2乗$x^2$が前節の不変な間隔の2乗です。

時空のベクトル$x$は、$x^2>0$なら**時間的**、$x^2=0$なら**光的**、$x^2<0$なら**空間的**と呼ばれます。

- **光円錐**：1つの事象から見て光的な差を持つ事象の集まりです。$x_0>0$の側（未来光円錐）は、その事象で出た光が届く事象、$x_0<0$の側（過去光円錐）は、その事象に届く光が出た事象です。
- **因果**：影響や情報を伝える信号は光より速くは進めない、という因果律のもとでは、ある事象が影響を及ぼせるのは、未来光円錐の内部と表面の事象だけです。差が空間的な2つの事象は互いに信号を送れないので、順序の判断が観測者によって異なっても、原因と結果が逆転することはありません。差が時間的または光的な事象の前後関係は、どの観測者にとっても同じです。
- **世界線と観測者**：粒子が時空に描く曲線を世界線と呼びます。質量を持つ粒子の速さは$c$より小さいので、その世界線の接ベクトルは時間的です（光の世界線の接ベクトルは光的です）。観測者は、自分の世界線の未来向きの単位接ベクトル（未来向きの単位時間的ベクトル）を時間方向とし、それに直交する方向を空間とします。本記事の$\gamma_0$は、静止した観測者の時間方向です。
...
観測者を1人選ぶことは、その観測者の世界線の向き、すなわち時間軸となる単位ベクトルを1つ選ぶことです。以下では慣性観測者を考え、一定の未来向き単位時間的ベクトルを時間軸に選びます。ここではそれを$\gamma_0$とします。時間軸で決まるのは時間と空間への分け方で、空間の中の3本の座標軸の向きは別に選べます。観測者にとっての空間は、時間軸に直交するベクトル（$\gamma_0\cdot x=0$）の全体で、$\gamma_1,\gamma_2,\gamma_3$が張ります。この時間軸$\gamma_0$を基準にして、時空の量を$\operatorname{Cl}_{3,0}(\mathbb R)$の量に分けます。
...
| | 制約 | 時間発展 |
|---|---|---|
| $D\cdot F$（源を持つ） | ガウスの法則 | アンペール＝マクスウェルの法則 |
| $D\wedge F$（源を持たない） | $\nabla\cdot\boldsymbol B=0$ | ファラデーの法則 |

各組に制約と時間発展が1本ずつ入ります。時間発展と制約への分け方には観測者の選択が要りますが、時空のベクトルとトリベクトルへの分け方には要りません。
