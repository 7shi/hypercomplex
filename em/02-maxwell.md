...
[[7shi-em1]]で述べたとおり、電荷は正負の符号を含めた総量が保存されます。領域$V$の中の電荷$Q=\int_V\rho\,dV$が変化するのは、境界$\partial V$を通って電荷が出入りするときだけです。境界を外向きに単位時間に通過する電荷は$\oint_{\partial V}\boldsymbol J\cdot\boldsymbol n\,dS$なので
...
ベクトル解析でマクスウェル方程式を書くときは、電場と磁場のそれぞれについて、発散を取るのか回転を取るのかを1本ずつ選び、時間微分の項をどちらの式に入れるかを決めて、4本の式を組み立てます。1本の式では、この選択がありません。$F$に$\mathcal D$を掛け、幾何積の規則$e_k^2=1$、$e_ke_l=-e_le_k$で展開してグレードごとにまとめるだけで、4本の式に必要な発散と回転が、必要な組み合わせで自動的に揃います。発散と回転は、作用させる演算子として選ぶものではなく、1つの微分の結果に現れる成分です。
...
伝わる速さ$c$は、[[7shi-em1]]の関係$\varepsilon_0\mu_0c^2=1$で定めた$c=1/\sqrt{\varepsilon_0\mu_0}$です。$\varepsilon_0$と$\mu_0$は、歴史的には静止した電荷の間の力と電流の間の力という、光とは無関係に見える実験から決まる定数でしたが、そこから計算した電磁波の速さは、測定されていた光の速さと一致しました。マクスウェルはこれをもとに、光は電磁波であると結論しました（1864年）。電磁波の存在は、1888年にヘルツが電気回路からの電磁波の発生と検出によって実験的に確かめています。現在では、電波・赤外線・可視光・紫外線・X線などは、振動数（波長）の違う電磁波として統一的に理解されています。
