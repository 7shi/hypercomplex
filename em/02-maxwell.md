...
[[7shi-em1]]で述べたとおり、電荷は正負の符号を含めた総量が保存されます。空間に固定した領域$V$の中の電荷$Q=\int_V\rho\,dV$が変化するのは、境界$\partial V$を通って電荷が出入りするときだけです。境界を外向きに単位時間に通過する電荷は$\oint_{\partial V}\boldsymbol J\cdot\boldsymbol n\,dS$なので
...
静的な式のベクトル部はアンペールの法則$\nabla\times\boldsymbol B=\mu_0\boldsymbol J$でした。回転の発散は$0$なので、この式では$\nabla\cdot\boldsymbol J=0$に限られ、$\partial_t\rho\ne0$となる場所（充電中のコンデンサーの板など）では連続の式と両立しません。
...
マクスウェルは、アンペールの法則の電流密度に**変位電流密度**$\varepsilon_0\partial_t\boldsymbol E$を加えてこの矛盾を解消しました。ガウスの法則$\nabla\cdot\boldsymbol E=\rho/\varepsilon_0$を時間で微分すれば$\nabla\cdot(\varepsilon_0\partial_t\boldsymbol E)=\partial_t\rho=-\nabla\cdot\boldsymbol J$なので、$\boldsymbol J+\varepsilon_0\partial_t\boldsymbol E$の発散はつねに$0$になります。コンデンサーの隙間では、変位電流密度を曲面で積分した**変位電流**が導線の電流$I$と等しくなり、どちらの曲面を選んでも結果が一致します。修正された式
...
&&&ex 充電中のコンデンサー
面積$A$の2枚の平行な板に電荷$Q$と$-Q$がたまっているとし、端の効果は無視します。充電は十分ゆっくりで、各時刻の板の間の電場は静電場の式で近似できるとします。[[7shi-em1]]の一様な面電荷の例より、板の間の電場は板に垂直で、大きさは$E=\sigma/\varepsilon_0=Q/\varepsilon_0A$です。板の間を通る曲面を貫く変位電流は

$$
I_{\mathrm d}=\varepsilon_0A\frac{dE}{dt}=\frac{dQ}{dt}=I
$$

で、導線の電流に等しくなります。これは電荷が隙間を横切って流れるということではありません。時間変化する電場が、磁場の回転の式で電流密度と同じ位置に入るということです。
&&&

## 電磁誘導

コイル（巻いた導線）に磁石を近づけたり遠ざけたりすると、電池をつながなくてもコイルに電流が流れます。ファラデーが発見したこの現象を**電磁誘導**と呼びます。ここではコイルは動かさず、磁石を動かすとします。曲面の法線と周回の向きは、[[7shi-em1]]と同じく右ねじの関係に取ります。コイルを縁とする曲面を貫く磁束$\Phi=\int_S\boldsymbol B\cdot\boldsymbol n\,dA$が時間変化すると、コイルに沿う電場の周回積分

$$
\mathcal E=\oint_{\partial S}\boldsymbol E\cdot d\boldsymbol x=-\frac{d\Phi}{dt}
$$

が生じます。$\mathcal E$を**起電力**と呼び、単位電荷を一周させるときに電場がする仕事にあたります。単位は電位差と同じボルトです。名前に「力」を含みますが力ではなく、静電場と違って電位の差としては表せません。負号は、閉じたコイルに流れる誘導電流が、磁束の変化を妨げる向きの磁場を作ることを表します（レンツの法則）。
...
時間微分が入るのはベクトル部と2ベクトル部だけです。この2本は、現在の場の空間変化から電場と磁場が次にどう変化するかを決める時間発展の式です。一方、スカラー部と擬スカラー部の2本は時間微分を含まず、その時刻の場が満たすべき制約です。電荷が保存されれば、初めに満たされた2つの制約は時間発展によって保たれます。実際、ベクトル部と連続の式から$\partial_t(\nabla\cdot\boldsymbol E-\rho/\varepsilon_0)=-(\partial_t\rho+\nabla\cdot\boldsymbol J)/\varepsilon_0=0$、2ベクトル部から$\partial_t(\nabla\cdot\boldsymbol B)=-\nabla\cdot(\nabla\times\boldsymbol E)=0$です。代数的に時間微分が入るグレードかどうかが、そのまま式の物理的な役割の違いになっています。

## 発散と回転を選ばない計算

ベクトル解析でマクスウェル方程式を書くときは、電場と磁場のそれぞれについて、発散を取るのか回転を取るのかを1本ずつ選び、時間微分の項をどちらの式に入れるかを決めて、4本の式を組み立てます。1本の式では、この選択がありません。$F$に$\mathcal D$を掛け、幾何積の規則$e_k^2=1$、$e_ke_l=-e_le_k$で展開してグレードごとにまとめるだけで、4本の式に必要な発散と回転が、必要な組み合わせで自動的に揃います。発散と回転は、作用させる演算子として選ぶものではなく、1つの微分の結果に現れる成分です。どの方程式が自然界を記述するかは、実験によって決まります。幾何代数は電磁気の法則を前提にした体系ではなく、ベクトルの計算を整理する代数として整えられたものです。その代数で電磁気の法則を書くと、法則に必要な発散と回転が1つの微分のグレード成分として過不足なく現れます。

同じことは、ベクトル解析で覚える恒等式にも当てはまります。スカラー関数$\varphi$とベクトル値の関数$\boldsymbol V$について、$D^2=\Delta$を[[7shi-em1]]の公式「ベクトルと2ベクトルの微分」で展開すると次のようになります。
...
左の式は固定した回路に対するファラデーの法則の積分形で、左辺は回路に沿う起電力です。右の式はアンペール＝マクスウェルの法則の積分形で、曲面を通る電流に、電束（電場の流束）の時間変化による変位電流が加わっています。領域の基本定理からは、ガウスの法則と閉曲面を通る磁束が$0$であることが[[7shi-em1]]と同じ形で得られます。
...
[1本の式](#thm-maxwell)に左から$\bar{\mathcal D}$を掛けます。左辺は$(\partial_0^2-\Delta)F$です。スカラーの作用素はグレードを変えないので、左辺の$(\partial_0^2-\Delta)F$はベクトル部と2ベクトル部しか持ちません。右辺を計算すると次のようになります。
...
が得られます。[1本の式](#thm-maxwell)の解については、電荷の保存を独立に仮定する必要はありません。逆に、源$\rho,\boldsymbol J$を与えて場を求める立場では、連続の式は解が存在するための必要条件です。ベクトル解析では、ガウスの法則の時間微分とアンペール＝マクスウェルの法則の発散を組み合わせ、恒等式$\nabla\cdot(\nabla\times\boldsymbol B)=0$を使って導く式です。ここでは、$F$がベクトルと2ベクトルだけを持つので、$(\partial_0^2-\Delta)F$はスカラー成分を持てません。電荷と電流を勝手に指定すると右辺にスカラー成分が生じ、それを消す条件が電荷の保存です。
...
電荷も電流もない真空では右辺が$0$になり、$\boldsymbol E$と$\boldsymbol B$の各成分は速さ$c$で伝わる波動方程式を満たします。電磁場の各成分が速さ$c$の波動方程式を満たすことが、1階の方程式$\mathcal DF=0$に共役$\bar{\mathcal D}$を掛けるという1回の操作から分かります。ベクトル解析では、ファラデーの法則とアンペール＝マクスウェルの法則の回転を取り、回転の回転の公式$\nabla\times(\nabla\times\boldsymbol E)=\nabla(\nabla\cdot\boldsymbol E)-\Delta\boldsymbol E$を電場と磁場のそれぞれに使って導きます。ここでは$\bar{\mathcal D}$を掛けるだけで、電場と磁場の式が同時に出ます。逆に、各成分が波動方程式を満たすだけでは、元のマクスウェル方程式を満たすとは限りません。

&&&rem 光は電磁波
波動方程式は、ある場所の乱れが一定の速さで周囲に伝わっていくことを表します。1方向だけの変化を考えると、任意の波形$f$について$f(z-ct)$は波動方程式の解です。同じ波形の位置は、時間$\Delta t$の間に$z$方向へ$c\Delta t$だけ移動します。電場と磁場は、一方が先に生じて他方を作るのではなく、マクスウェル方程式で結ばれた1つの場$F$として同時に変化します。その結びつきによって、電荷や電流から離れた場所でも場の変化が波として伝わります。これが**電磁波**です。

伝わる速さ$c$は、[[7shi-em1]]の関係$\varepsilon_0\mu_0c^2=1$で定めた$c=1/\sqrt{\varepsilon_0\mu_0}$です。$\varepsilon_0$と$\mu_0$は、歴史的には静止した電荷の間の力と電流の間の力という、光とは無関係に見える実験から決まる定数でしたが、そこから計算した電磁波の速さは、測定されていた光の速さと一致しました。マクスウェルは1861〜62年の論文でこの一致を指摘し、光は電磁的な波であると論じました。1864年に発表した論文（刊行は1865年）では、これを電磁場の理論としてまとめています。電磁波の存在は、1887〜88年にヘルツが電気回路からの電磁波の発生と検出によって実験的に確かめています。現在では、電波・赤外線・可視光・紫外線・X線などは、振動数（波長）の違う電磁波として統一的に理解されています。

[[7shi-em1]]では、$\boldsymbol E$と$c\boldsymbol B$の単位が揃うことを、電場と磁場をまとめる理由として使いました。そこで現れた$c$の正体が、電磁波の速さです。
&&&

[[7shi-cla4]]の調和関数と違い、ここで現れるのは時間方向に伝わる波を許す方程式です。
