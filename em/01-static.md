...
電荷は、物体が電気的な力を受けたり及ぼしたりする度合いを表す量で、正と負があります。$\mathrm{SI}$単位系での単位はクーロン（$\mathrm C$）です。電荷は、正負の符号を含めた総量が保存されます。ある領域の電荷の総量が変わるのは、境界を通って電荷が出入りするときだけです（電荷の保存）。
...
電荷の流れを**電流**と呼びます。導線を流れる電流$I$は、導線の断面を単位時間に通過する電荷で、単位はアンペア（$\mathrm A=\mathrm{C/s}$）です。空間に広がった電流は**電流密度**$\boldsymbol J$で表します。電流の向きは正の電荷が動く向きと定め、負の電荷の移動は逆向きの電流として数えます。$\boldsymbol J$はこの電流の向きを向き、その大きさは、流れに垂直な単位面積を単位時間に通過する電荷です（単位$\mathrm{A/m^2}$）。電荷密度$\rho$の1種類の電荷が同じ速度$\boldsymbol v$で動いていれば$\boldsymbol J=\rho\boldsymbol v$で、$\rho<0$なら$\boldsymbol J$は$\boldsymbol v$と逆向きです。何種類かの電荷が動いていれば、それぞれの寄与を足します。

電流の流れる2本の平行な導線は、同じ向きの電流なら引き合い、逆向きなら反発します。これは静止した電荷の間のクーロン力とは別の力で、動いている電荷だけに働きます。平行な導線の間の力は、このような磁気的な力の一例です。動く電荷に働く力をさらに調べると、速度$\boldsymbol v$で動く電荷$q$には、電場による力に加えて、空間の各点で定まるベクトル$\boldsymbol B$を使って$q\boldsymbol v\times\boldsymbol B$と表される力が働くことが分かります。この$\boldsymbol B$を**磁場**と呼びます。電場と同じく、この関係が磁場の定義を与えます。単位はテスラ（$\mathrm T$）です。
...
$\varepsilon_0$は静止した電荷の間の力の法則に、$\mu_0$は電流の間の力の法則に現れる定数なので、この関係は静的な現象だけからは説明がつきません。本記事では、$\boldsymbol E$と$c\boldsymbol B$が同じ単位（$\mathrm{V/m}$）を持つという事実を使って、電場と磁場を1つの量にまとめます。
...
電荷密度$\rho$と電流密度$\boldsymbol J$は$C^1$級で、ある有界な領域の外で$0$になるとします。点電荷や細い導線は、この仮定の下では小さな領域に集中した分布の理想化として扱います。場は与えた電荷と電流が作るものだけを考え、無限遠で$0$に近づくものとします（外から加えた一様な場などは重ねません）。積分は$\boldsymbol y$について全空間で取り、$dV$は$\boldsymbol y$の体積要素です。
...
面として表した磁場は、ローレンツ力の向きもそのまま表します。2ベクトル$i\boldsymbol B$と速度$\boldsymbol v$の積の反対称部分を取ると

$$
\boldsymbol v\times\boldsymbol B=\frac12\bigl((i\boldsymbol B)\boldsymbol v-\boldsymbol v(i\boldsymbol B)\bigr)
$$

です。面$i\boldsymbol B$に垂直な速度成分（$\boldsymbol B$に平行な成分）は$i\boldsymbol B$と可換なので右辺に寄与せず、磁気的な力を受けません。面内の速度成分は$i\boldsymbol B$と反可換なので右辺は$(i\boldsymbol B)\boldsymbol v$となり、速度を面内で直角に回した向きの力になります。磁場は運動の向きを面内で曲げるだけで、速さを変えません。
...
電場の発散と回転、磁場の発散と回転は、ベクトル解析では別々の4本の式ですが、ここでは$D$を1回作用させた結果の4つのグレードです。どの場に発散を取り、どの場に回転を取るかを選ぶ必要はなく、$D$を掛けて幾何積で展開すれば、4本の式に必要な微分が自動的に揃います。源を持つのはスカラー部とベクトル部だけで、2ベクトル部と擬スカラー部が$0$になるのは源$\rho-\boldsymbol J/c$に2ベクトルと擬スカラーの成分がないためです。磁気単極子があればその密度は擬スカラーの位置に入ります。物理的な内容は、出発点とした2つの実験則と定常電流の条件に入っています。幾何代数は、その内容を変えずに、必要な微分を1つの積のグレード成分として整理します。
...
&&&ex 点電荷の電場
電荷$Q$が原点のまわりに球対称に分布し、半径$a$の球の内部に収まっているとします。対称性から、電場は動径方向を向き、$\boldsymbol E=E_r(r)\,\boldsymbol x/r$と書けます。動径成分$E_r(r)$は原点からの距離$r$だけで決まります。$r>a$の半径$r$の球面を$\partial V$に取ると、$\boldsymbol E\cdot\boldsymbol n=E_r(r)$が球面上で一定なので
$$
4\pi r^2E_r(r)=\frac Q{\varepsilon_0},\qquad E_r(r)=\frac Q{4\pi\varepsilon_0r^2}
$$
です。$Q>0$なら電場は外向き、$Q<0$なら内向きです。球の外の電場は、全電荷$Q$が原点に集まった点電荷の電場と同じで、[点電荷のクーロン力](#fml-coulomb-force)と一致します。$a\to0$の極限が点電荷です。
&&&

&&&ex 一様な面電荷の電場
十分に広い平面に、単位面積あたり$\sigma>0$（単位$\mathrm{C/m^2}$）の電荷が一様に分布しているとします。これも点電荷と同じく、薄い層に集中した分布の理想化です。対称性から、電場は面に垂直で、面の両側で面から遠ざかる向きを向き、両側で大きさ$E$が等しくなります。面を貫く断面積$A$の薄い円柱を$\partial V$に取ると、側面では$\boldsymbol E\cdot\boldsymbol n=0$で、2つの底面からそれぞれ$EA$の流束が出るので
$$
2EA=\frac{\sigma A}{\varepsilon_0},\qquad E=\frac\sigma{2\varepsilon_0}
$$
です。電場の大きさは面からの距離によりません。電荷密度$\sigma$と$-\sigma$の2枚の平行な面では、それぞれの電場を重ね合わせると、面の間では同じ向きに加わって大きさ$\sigma/\varepsilon_0$になり、外側では打ち消し合って$0$になります。無限に広い面は電荷の台が有界という仮定を満たさないので、面の広さに比べて面に近く、端から離れた場所での近似として読みます。
&&&
...
&&&ex 直線電流の磁場
$e_3$軸に沿った十分長い直線の導線に、$e_3$の向きに電流$I$が流れているとします。対称性から、磁場は導線を軸とする円の接線の向きを向き、その大きさ$B(r)$は導線からの距離$r$だけで決まります。導線の半径より大きい$r$について、導線を中心とする半径$r$の円を$\partial S$、それが囲む円板を$S$に取ると、円板を貫く電流は導線の全電流$I$なので
$$
2\pi rB(r)=\mu_0I,\qquad B(r)=\frac{\mu_0I}{2\pi r}
$$
です。磁場の向きは、電流の向きに右ねじを進めるときの回転の向きです。無限に長い導線は電流密度の台が有界という仮定を満たさないので、閉じた回路の一部をなす十分長い直線部分について、導線の太さより十分大きく、端や戻りの導線までの距離より十分小さい$r$での近似として読みます。
&&&

微分形の4本の式が$DF$のグレード成分だったのと同じく、積分形の式は2つの基本定理のグレード成分として並びます。電荷密度は体積で足し合わせると総電荷になり、電流密度は曲面を通る流れとして足し合わせると電流になります。これに対応して、電場の源を数えるガウスの法則は体積と閉曲面の関係に、磁場の源を数えるアンペールの法則は曲面と閉曲線の関係になります。源のグレードの違い（電荷密度はスカラー、電流密度はベクトル）が、この違いとして現れています。
