...
$ρ_\pm=\Psi_\pm\Psi_\pm^\dagger$から計算したブロッホベクトルは、$ρ$のブロッホベクトル$\boldsymbol r$と同じ向きか逆向きかのいずれかになります。そのため、行列$ρ$からブロッホベクトル$\boldsymbol r$が既に求まっていれば、固有ベクトル$\Psi_\pm$を改めて計算する必要はありません。
&&&
...
密度行列が表す系に対して、ある方向を選んで**測定**を行えば、その方向を「向いている・向いていない」に対応する二値の結果（$+1$または$-1$）が確率的に得られます。この節では、多数回測定したときの平均である**期待値**から始め、そこから1回ごとの結果の確率を取り出し、最後に確率を状態ベクトルから直接求める規則へと進みます。
...
この内積は行列の側でも計算できます。方向$\boldsymbol n$を行列$A=\boldsymbol n\cdot\boldsymbol σ$で表し、$\mathrm{tr}\,σ_i=0$と$\mathrm{tr}(σ_iσ_j)=2\delta_{ij}$を使えば、密度行列$ρ$との積のトレースが内積を与えます。
...
期待値は、1回の測定結果そのものではありません。$z$軸方向の測定を例に説明します。

1回ごとの結果は$+1$または$-1$のどちらかの値として確率的に観測されます。同じ状態を多数回測定したときの平均が期待値$z$です。

&&&rem 状態の収縮
結果$+1,-1$に対応する状態ベクトルは、ブロッホベクトルが北極・南極を指す純粋状態、つまり$\Psi_\uparrow,\Psi_\downarrow$です。物理では、測定で得られた結果に応じて、状態ベクトルは$\Psi_\uparrow$または$\Psi_\downarrow$へ収縮すると考えます。そのため同じ測定を連続で行えば、2回目以降は状態が固定されることになるため、1回目と同じ結果が得られます。

したがって、上で述べた「同じ状態を多数回測定」とは、この意味での連続測定ではありません。期待値を得るには、同じ状態$\Psi$を独立に何度も用意し、そのたびに1回だけ測定するという手続きが必要です。
&&&

$+1$が出る確率を$p_+$、$-1$が出る確率を$p_-$とすれば、期待値は「値×確率」の総和として定義されるので

$$
z=(+1)\,p_+ +(-1)\,p_-=p_+-p_-
$$

が成り立ちます。これに確率の規格化条件$p_++p_-=1$を合わせた連立方程式を解けば、期待値から確率が求まります。
...
この変換は、期待値の範囲$[-1,1]$を確率の範囲$[0,1]$へ写すスケール調整に相当します。北極（$z=1$）なら確実に$+1$、南極（$z=-1$）なら確実に$-1$、中心（$z=0$）なら半々です。

&&&rem 三角関数で見る2乗
極角$θ$を使って$z=\cosθ$とおけば、半角の公式$\cos^2\dfracθ2=\dfrac{1+\cosθ}2$より
$$
p_+=\frac{1+\cosθ}2=\cos^2\fracθ2
$$
となります。一見線形なスケール調整に見えるこの変換は、$\cos$を2乗の形に変換していることになります。
&&&

確率$p_+,p_-$は、状態ベクトル$\Psi=(α,β)$の成分からも計算できます。
...
&&&rem
$α=\cos\dfracθ2$とおけば、$|\Psi_\uparrow^\dagger\Psi|=|α|=\cos\dfracθ2$となります。よって$p_+=|\Psi_\uparrow^\dagger\Psi|^2=\cos^2\dfracθ2$となり、先ほど三角関数で確認した式と一致します。つまり、半角の公式による2乗は、内積の絶対値の2乗に対応します。
&&&
