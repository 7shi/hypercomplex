前回は、状態ベクトルとパウリ行列からブロッホベクトルと密度行列を導出しました[[7shi-s]]。今回は、パウリ行列によって表現される純粋状態と混合状態を説明します。量子情報への接続は予備知識程度に留めます。
...
密度行列が表す系に対して、ある方向を選んで**測定**を行えば、その方向を「向いている・向いていない」に対応する二値の結果（$+1$または$-1$）が確率的に得られます。この節では、多数回測定したときの平均である**期待値**から始め、そこから1回ごとの結果の確率を取り出します。
...
&&&rem 状態の収縮
例えば$\boldsymbol n$が$z$軸方向なら、結果$+1,-1$に対応する状態は、ブロッホベクトルが北極・南極を指す純粋状態の密度行列、つまり$ρ_\uparrow=\frac12(\mathrm I+σ_z),\ ρ_\downarrow=\frac12(\mathrm I-σ_z)$です。物理では、測定で得られた結果に応じて、密度行列は$ρ_\uparrow$または$ρ_\downarrow$へ収縮すると考えます。そのため同じ測定を連続で行えば、2回目以降は状態が固定されることになるため、1回目と同じ結果が得られます。
...
この変換は、期待値の範囲$[-1,1]$を確率の範囲$[0,1]$へ写すスケール調整に相当します。$\boldsymbol r$が$\boldsymbol n$と同じ向き（$\boldsymbol r\cdot\boldsymbol n=1$）なら確実に$+1$、逆向き（$\boldsymbol r\cdot\boldsymbol n=-1$）なら確実に$-1$、直交または原点（$\boldsymbol r\cdot\boldsymbol n=0$）なら半々です。

&&&rem 三角関数で見る2乗
純粋状態（$|\boldsymbol r|=1$）に限れば、$\boldsymbol r$と$\boldsymbol n$のなす角$θ$を使って$\boldsymbol r\cdot\boldsymbol n=\cosθ$と書けます。半角の公式$\cos^2\dfracθ2=\dfrac{1+\cosθ}2$より
$$
