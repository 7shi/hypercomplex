...
- [memo-vector-fields.py](memo-vector-fields.py) — MEMO.md の話題1・2の検証。$w=\beta/\alpha=\tan(\theta/2)e^{i\varphi}=(n_1+in_2)/(1+n_3)$が南極からの立体射影であること、生成子$X$による$w$の微小変換$c+(d-a)w-bw^2$（メビウス変換と指数関数からの再導出）、$-i\sigma_a/2$に対応する正則ベクトル場が$d\boldsymbol n/dt=\boldsymbol e_a\times\boldsymbol n$を再現すること、昇降演算子の像$\partial_w,\ -w^2\partial_w$が実ベクトル場ではないこと、球面座標でのキリング場と$[X_a,X_b]=-\varepsilon_{abc}X_c$、$L_a=-i\mathcal{L}_{X_a}$の交換関係と$L_3,L_\pm$の具体形、$L^2=-\Delta_{S^2}$と$L^2Y_l^m=l(l+1)Y_l^m$（$l\le2$）の検証。
- [memo-spin-j.py](memo-spin-j.py) — MEMO.md のスピン$j$表現と縮約の検証。$J_+=\partial_w,\ J_3=j-w\partial_w,\ J_-=2jw-w^2\partial_w$の交換関係・カシミール$j(j+1)$・ウェイト基底と梯子の端（$j\le2$）、$F(\alpha,\beta)=\alpha^{2j}g(w)$から演算子が導かれることと反変性（$\rho(\sigma_\pm)=-J_\mp$）、$j=1/2$で$\{1,w\}\leftrightarrow\{\alpha,\beta\}$の行列が$\sigma_3/2,\sigma_\pm$に一致すること、$u=\sqrt{2j}\,w$による$j\to\infty$の縮約で$[a,a^\dagger]=1$（掛け算と微分）に至ることの検証。
