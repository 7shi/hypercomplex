...
- [memo-mobius.py](memo-mobius.py) — MEMO.md の「行列が微分になる」部分の検証。$w=\beta/\alpha=\tan(\theta/2)e^{i\varphi}=(n_1+in_2)/(1+n_3)$が南極からの立体射影であること、生成子$X$による$w$の動き$c+(d-a)w-bw^2$（メビウス変換と指数関数からの再導出）、$-i\sigma_a/2$に対応する係数が$d\boldsymbol n/dt=\boldsymbol e_a\times\boldsymbol n$を再現すること、昇降演算子の像$\partial_w,\ -w^2\partial_w$がブロッホ球の回転としては存在しないことの検証。
