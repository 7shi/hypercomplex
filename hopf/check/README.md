...
- [05-two-qubit.py](05-two-qubit.py) — $\mathbb{C}^4\cong\mathbb{H}^2$の詰め込みと$q_1q_2^*$の成分公式、分離可能⟺$\alpha\delta-\beta\gamma=0$⟺$\mathbf j,\mathbf k$成分の消失、関係式$x_A^2+y_A^2+z_A^2+C^2=1$、ベル状態の像と全体位相、ファイバー（右乗算$q=u+v\mathbf j$が状態には$\mathrm I_2\otimes\begin{pmatrix}u&-v^*\\v&u^*\end{pmatrix}$として働くこと）の検証。
- [05-three-qubit.py](05-three-qubit.py) — $\mathbb{C}^4\cong\mathbb{O}$の複素基底$\{1,e_2,e_4,e_6\}$による詰め込みと左$\mathbb{C}$-線形性、A|BC分離可能⟺$e_2,\dots,e_7$成分の消失、GHZ・W状態の像、像の複素成分$\pi_{\mathbb{C}}(o_1o_2^*)=\sum_ku_kv_k^*$、像の方向が不変量でないこと（$e^{i\pi/4}\mathrm{GHZ}$と$B$・$AC$の積状態が同じ$(e_7;\ 0)$に写る）、ファイバー$S^7$の分離可能性、$e_1$を固定する自己同型群$\operatorname{SU}(3)$の検証。
