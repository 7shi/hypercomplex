...
N = 32
E32 = [basis(N, i) for i in range(N)]


def derivation_dim(n):
    """dim Der of the dimension-n Cayley-Dickson algebra (via the Gram matrix)."""
    En = [basis(n, i) for i in range(n)]
    prod = [[mul(En[i], En[j]) for j in range(n)] for i in range(n)]
    G = np.zeros((n * n, n * n))
    for i in range(1, n):
    ...
    return int(np.sum(w < 1e-9 * w.max()))


print("dim Der(32-ion) =", derivation_dim(N), "(unchanged: continuous part stays g2)")


def is_aut(M, n):
    En = [basis(n, i) for i in range(n)]
    return all(np.allclose(M @ mul(En[i], En[j]), mul(M @ En[i], M @ En[j]))
               for i in range(n) for j in range(n))


def eps_of(n):
    h = n // 2
    return np.block([[np.eye(h), np.zeros((h, h))], [np.zeros((h, h)), -np.eye(h)]])


def psi_of(n):
    """Rotate each plane (q, q e_h), q imaginary in the half algebra, by 2pi/3."""
    h = n // 2
    En = [basis(n, i) for i in range(n)]
    M = np.zeros((n, n))
    M[0, 0] = M[h, h] = 1.0
...
eps32, psi32 = eps_of(N), psi_of(N)
print("new eps, psi are automorphisms of the 32-ions:",
      is_aut(eps32, N) and is_aut(psi32, N),
      "/ psi^3 = 1, eps psi eps = psi^2:",
      np.allclose(np.linalg.matrix_power(psi32, 3), np.eye(N))
      and np.allclose(eps32 @ psi32 @ eps32, np.linalg.matrix_power(psi32, 2)))

eps_l, psi_l = lift(eps), lift(psi)
print("diagonal lifts of the sedenion eps, psi are automorphisms:",
      is_aut(eps_l, N) and is_aut(psi_l, N),
      "/ commute with the new eps, psi:",
      all(np.allclose(A @ B, B @ A)
          for A in (eps32, psi32) for B in (eps_l, psi_l)))

gens = [eps32, psi32, eps_l, psi_l]
elems = [np.eye(N)]
frontier = [np.eye(N)]
while frontier:
    nxt = []
    for M in frontier:
        for g in gens:
            P = g @ M
            if not any(np.allclose(P, Q) for Q in elems):
                elems.append(P)
                nxt.append(P)
    frontier = nxt
print("group generated by the two S3's:", len(elems), "elements (= |S3 x S3| = 36)")
