...
$$
になります。これは前回見た複素数$a+bi$の行列表現、すなわち$\operatorname{SO}(2)$の元にほかなりません。逆に言えば$\operatorname{SU}(2)$は、$\operatorname{SO}(2)$の成分$a,b$をそれぞれ複素数$z,w$に置き換えて**複素化**したもの（正確には、共役を含むユニタリ条件のもとで複素成分へ拡張したもの）と見ることができます。本記事では、この素朴な意味で「複素化」と呼びます。
