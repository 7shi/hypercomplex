...
単位四元数$q$を使って、四元数$x$を挟み込む変換を考えます。これを**共役作用**と呼び、回転を表すため$\rho_q$と表記します。
...
$q=\exp(i\theta/2)$として、左作用$L_q(x)$は2つの平面$(1,i)$も$(j,k)$も同じ向きに角度$\theta/2$だけ回転します。

$$
L_q(x) = qx = qz + qw\,j = \exp\left(i\frac{\theta}{2}\right)z + \exp\left(i\frac{\theta}{2}\right)w\,j
$$

右作用$R_{q^{-1}}(x)=xq^{-1}$も等傾回転ですが、$(1,i)$平面では逆向きに角度$-\theta/2$で回転します。

$$
R_{q^{-1}}(x) = xq^{-1} = zq^{-1} + wjq^{-1} = \exp\left(-i\frac{\theta}{2}\right)z + \exp\left(i\frac{\theta}{2}\right)w\,j
$$

共役作用$\rho_q=L_q\circ R_{q^{-1}}$はこの2つの合成なので、平面ごとに角度が足し合わされ、$(1,i)$平面は$\theta/2-\theta/2=0$、$(j,k)$平面は$\theta/2+\theta/2=\theta$となります。左右で挟むことで、行列式$1$に由来する「両平面が同じ角度だけ回る」という制限は相殺され、実軸を固定した2倍の角度の次元回転として現れます。$q$の位相を半角$\theta/2$で与えるのは、この相殺と倍加の帰結です。
