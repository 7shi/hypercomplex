...
- リー代数は通常の積については一般には閉じていないが、括弧積$[x,y]=xy-yx$については閉じている
...
$$
A = P \begin{pmatrix} 2\pi i & 0 \\ 0 & -2\pi i \end{pmatrix} P^{-1}
= \begin{pmatrix} 2\pi i & -4\pi i \\ 0 & -2\pi i \end{pmatrix}
$$
...
$$
になります。これは前回見た複素数$a+bi$の行列表現、すなわち$\operatorname{SO}(2)$の元にほかなりません。逆に言えば$\operatorname{SU}(2)$は、$\operatorname{SO}(2)$の成分$a,b$をそれぞれ複素数$z,w$に置き換えたものと見ることができます。ただしこのとき、$\operatorname{SO}(2)$の直交条件$A^\mathsf{T}A=I$の転置を、エルミート共役に読み替えています。この読み替えは次のように正当化できます。
...
したがって、実行列として直交条件を課すことは、複素行列としてユニタリ条件$A^\dagger A=I$を課すことに当たります。これに行列式$1$の条件を合わせれば$\operatorname{SU}(2)$が得られます。

このように、成分を複素数に置き換えるとともに直交条件をユニタリ条件に読み替える操作を、本シリーズでは**複素ユニタリ化**と呼びます。
...
&&&rem
「$\operatorname{SO}(2)$の複素ユニタリ化としての$\operatorname{SU}(2)$」ではベクトルへの作用を想定していたことに注意が必要です。
