これまでに、複素数と四元数からリー群$\operatorname{U}(1)$と$\operatorname{Sp}(1) \cong \operatorname{SU}(2)$を構成し[[7shi-lie1]][[7shi-lie2]]、クリフォード代数とスピン群による一般化を経てきました[[7shi-lie3]][[7shi-lie4]]。今回は新しいリー群は登場しません。代わりに、これまでの記事で暗黙のうちに使ってきた「リー群とリー代数の対応」そのものを主題として、2つの道具を整備します。1つは、方向の異なる元で崩れる指数法則のずれを括弧積の言葉で完全に記述する**ベイカー・キャンベル・ハウスドルフの公式**（BCH公式）、もう1つは、たびたび登場した共役作用をリー代数の言葉で捉え直す**随伴作用**です。引き続き、数学的厳密さよりも直観性を重視します。
...
以前見た例を思い出します。$\exp(i\pi/2) = i$と$\exp(j\pi/2) = j$の積は$ij = k$ですが、引数を足した$\exp((i+j)\pi/2)$は$k$と一致しませんでした。方向の異なる元に対して、指数法則$\exp(x)\exp(y) = \exp(x+y)$は崩れます。
...
&&&rem 収束の制約がない
BCH級数には収束範囲の制約がありましたが、この閉形式は任意の$\alpha, \beta$で成り立ちます。リー群が数体系の中に住んでいる（基底の積が実線形結合の中で閉じている）ことの恩恵です。基底の積が閉じた数体系を持たないリー群では、この恩恵は失われ、級数と一般論に頼ることになります。
&&&
...
によって括弧積の言葉で完全に記述されます。高次項はすべて括弧積の入れ子で書けるため、リー代数（ベクトル空間と括弧積の組）を知れば単位元近傍での群の積が復元できます。リー代数が見るのは局所構造だけですが（二重被覆の例で見た通り）、逆に局所構造は括弧積で完全に決まります。応用として、群の往復$\exp(x)\exp(y)\exp(-x)\exp(-y) = \exp([x,y] + \cdots)$は括弧積を群のレベルで取り出します。また$\mathfrak{sp}(1) \cong \mathfrak{su}(2)$では、四元数の積がBCH級数を経ない厳密な合成則を与え、球面三角法の余弦定理につながります。
