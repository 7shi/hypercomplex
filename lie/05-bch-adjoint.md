...
と定めます（$[x^1] = x$、$[x^2y^1] = [x,[x,y]]$など）。各項の次数は$\sum_i(r_i+s_i)$で決まります。同じ次数の項を集めて整理すると、先ほどのBCH公式の係数が得られます。4次の項はヤコビ恒等式を使うと$-\frac{1}{24}[y,[x,[x,y]]]$の1項にまとめられますが、5次以降は入れ子の組み合わせ方が増え、項数が急速に増えます。
...
左辺は群における非可換性を検出する式です。単位元から出発して右から元を掛け足していく経路として読むと、$x$方向に進み、$y$方向に進み、来た道を逆順ではなく同順で戻ることに当たります。可換なら出発点に戻りますが、非可換なら戻りきれません。その閉じ残りが、2次の精度でちょうど括弧積$[x,y]$の方向に現れます。
...
角度には周期による不定性がありますが、ここでは$0<\gamma<\pi$の枝を選びます。この枝では$\gamma = \arccos a$とも書けます。$V = 0$の場合は積が$\pm1$となり、方向$w$は定まりません（任意に取れます）。
...
これは任意の実数$\alpha, \beta$について成り立つ積の公式ですが、球面三角形の辺長としての解釈を与えると球面余弦定理になります。球面三角形として解釈するため、ここでは$0<\alpha,\beta<\pi$とし、$u, v$は平行でないとします。各辺には最短の大円弧を取ります。単位四元数全体は3次元球面$S^3$なので、$1,\ \exp(u\alpha),\ \exp(u\alpha)\exp(v\beta)$の3点を頂点とする球面三角形が描けます。単位四元数を掛ける操作は$S^3$上の等長変換なので、$\exp(u\alpha)$から積までの距離は$1$から$\exp(v\beta)$までの距離に等しく、3辺の長さはそれぞれ$\alpha, \beta, \gamma$です。
...
BCH公式との整合も確認できます。BCH公式が与えるのは積そのものの虚部ではなく指数の引数$z = w\gamma$ですが、上の表示から$V \ne 0$のとき
...
となり、BCH公式の2次項と一致します。BCH級数の先頭部分が、四元数では外積の項として現れているわけです。
...
&&&rem 相似変換との関係
$\operatorname{Ad}_g(x) = gxg^{-1}$は、線形代数の相似変換$A \mapsto PAP^{-1}$の共役元$P$を$G$の元に限定したものです。相似変換が積を保つ性質から、後で見るように括弧積の保存も従います。一方、$\mathfrak{g}$からはみ出さないのは$\mathfrak{g}$が単位元での接空間だからです。
&&&
...
となります。群の元の共役は、指数写像の引数への随伴作用に翻訳されます。一般の3つの積をBCH公式で展開すると無限級数になりますが、共役という特別な組み合わせでは、BCH級数の収束条件によらず、この恒等式が厳密に成り立ちます。
...
このライプニッツ則は、「$\operatorname{Ad}_g$が括弧積を保つ」という性質の無限小版として導くこともできます。$g = \exp(tx)$として$\operatorname{Ad}_g[y,z] = [\operatorname{Ad}_g y, \operatorname{Ad}_g z]$の両辺を$t = 0$で微分すると、右辺は積の微分法則によって2項に分かれ、ちょうど上の式になります。ヤコビ恒等式は、共役作用が括弧積を保つことの無限小版でもあるのです。
...
となり、右辺は$x$軸周りの角度$\theta$の回転行列です。以前は$\rho_q(j) = j\cos\theta + k\sin\theta$のような成分計算で確認しましたが、その背後には$\operatorname{Ad}$と$\operatorname{ad}$をつなぐ一般的な仕組みが働いていたのです。以前、共役作用の半角として説明した基底の$1/2$も、ここでは基底の対応$\operatorname{ad}_{i/2} = J_x$を通して、随伴表現の一般論の中に位置付けられます。
...
$\operatorname{Ad}_q$が恒等変換になるのは、$qxq^{-1} = x$、つまり$q$がすべての純虚四元数と可換になるときです。$q = a + v$（$a$は実部、$v$は純虚部）とすると、純虚四元数$x$に対して
$$
[q, x] = [v, x] = 2\,(v \times x)
$$
であり、これがすべての$x$について$0$になるのは$v = 0$のときだけです。よって$q$は実数であり、単位四元数の中で実数は$q = \pm 1$の2つだけです。

$$
\ker \operatorname{Ad} = \{\pm 1\}
$$

像は$\operatorname{SO}(3)$全体であり、核が2元なので、対応は2対1です。以前は「$q$と$-q$が同じ回転を与える」ことを式変形で見ましたが、随伴作用の言葉では「核が$\{\pm 1\}$である」という構造的な事実に整理されます。
...
ヤコビ恒等式にも、随伴表現の言葉での3つ目の読み方があります。
...
&&&rem 随伴表現とルート
随伴表現は、リー代数の分類理論の主役でもあります。複素半単純リー代数では、カルタン部分代数$\mathfrak{h}$の元$h$による作用$\operatorname{ad}_h$を同時に調べます。$0 \ne v \in \mathfrak{g}$がすべての$h \in \mathfrak{h}$について$[h, v] = \alpha(h)\,v$を満たすとき、この非零の線形汎関数$\alpha \in \mathfrak{h}^*$を**ルート**と呼びます。ルートは分類の基本データです。
&&&
...
入れ子の括弧積を律するのがヤコビ恒等式$[x,[y,z]] + [y,[z,x]] + [z,[x,y]] = 0$です。行列や四元数の括弧積では、証明は結合法則を使った展開だけで済み、ライプニッツ則の形にも、「$\operatorname{ad}$が括弧積を交換子に写す」という主張にも読み替えられます。
