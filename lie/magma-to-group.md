...
マグマにおいて方程式$ax = b$と$ya = b$がそれぞれ一意に解ければ**準群**、準群が単位元を持てば**ループ**、ループが結合的であれば**群**と呼びます。この一意可解性を**可除性**と呼びます。準群には単位元を仮定しないため、可除性は逆元の存在とは別の条件です。

&&&
$$
(\text{集合},\text{演算})
\xrightarrow{\text{閉性}} \text{マグマ}
\xrightarrow{\text{可除性}} \text{準群}
