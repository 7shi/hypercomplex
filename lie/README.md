...
- **壮大な伏線回収**: 後半の06以降では八元数の対称性を扱います。結合律の崩れによって左右の作用が複雑に絡み合い、そこにクリフォード代数が生成する $\operatorname{Spin}(6)$ や $\operatorname{Spin}(7)$ といった群が立ち現れます。そこで $\operatorname{Spin}$ 群の作用を受けるのは八元数自身がなす8次元のスピノルであり、その部分群として前半で構成した $\operatorname{SU}(3)$ や $G_2$ が再登場し、すべての物語が統合されます。ただし、これは現時点での構想であり、実際の記事構成は変更される可能性があります。
