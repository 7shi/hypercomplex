...
axis-angle rotation R(u,theta), with rho_{-q} = rho_q, and the axis-to-unit
correspondence of 02-su2-so3.md (e_z <-> k, so exp(k theta/2) is the z-axis
lift while exp(i theta/2) is the x-axis one); the belt-trick homotopy
H_s(theta) = R(n(s), theta) with n(s) = (sin s, 0, cos s) staying a loop for
all s (H_s(0) = H_s(2pi) = I), connecting the forward 2pi loop (s=0) to the
...
# axis-to-unit correspondence: e_z <-> k, e_x <-> i (02-su2-so3.md)
print("exp(k theta/2) lifts R(e_z,theta):",
      np.allclose(rot_from_q(quat(ez, theta)), rot(ez, theta)) and
      np.allclose(quat(ez, theta), np.cos(theta/2)*I2 + np.sin(theta/2)*qk))
print("exp(i theta/2) lifts R(e_x,theta), not R(e_z,theta):",
      np.allclose(rot_from_q(np.cos(theta/2)*I2 + np.sin(theta/2)*qi),
                  rot([1, 0, 0], theta)) and
      not np.allclose(rot_from_q(np.cos(theta/2)*I2 + np.sin(theta/2)*qi),
                      rot(ez, theta)))

# belt-trick homotopy H_s(theta) = R(n(s), theta): tilt the axis from e_z to -e_z
def axis(s):
    return np.array([np.sin(s), 0.0, np.cos(s)])

ss = np.linspace(0, np.pi, 61)
ths = np.linspace(0, 2*np.pi, 121)
print("H_s(0) = H_s(2pi) = I for all s:",
      all(np.allclose(rot(axis(s), th), np.eye(3)) for s in ss for th in (0, 2*np.pi)))
print("H_0(theta) = R(e_z,theta):",
      all(np.allclose(rot(axis(0), th), rot(ez, th)) for th in ths))
print("H_pi(theta) = R(e_z,2pi-theta):",
      all(np.allclose(rot(axis(np.pi), th), rot(ez, 2*np.pi - th)) for th in ths))
grid = np.array([[rot(axis(s), th) for th in ths] for s in ss])
...
lifts = np.array([[quat(axis(s), th) for th in ths] for s in ss])
...
print("lift endpoint stays -1 for all s:",
      all(np.allclose(quat(axis(s), 2*np.pi), -I2) for s in ss))
