...
本稿では、この$\varepsilon$から「無限小」という解釈を取り払い、$\varepsilon^2=0$を満たす形式的な基底として正面から採用します。実数にこの$\varepsilon$を添加した数が二重数、四元数の係数を二重数に取り替えた数が二重四元数 (dual quaternion) です。二重四元数を使うと回転に加えて並進が扱えるようになり、3次元の剛体変換（回転と並進の合成）が、回転子と同じ「挟み込み」の形で統一的に表現できます。無限小三角形の計算は近似ではなく厳密な恒等式に昇格し、接空間の縛りが外れて、一般の空間での並進が手に入ります。
...
特に指数関数では、次の関係が**厳密に**成り立ちます。

$$
e^{\varepsilon y} = 1 + \varepsilon y
$$

テイラー展開の1次までの近似ではなく、2次以降の項が文字通り$0$になるため、等号で結べることに注意してください。この「1次で打ち切っても誤差が出ない」性質こそ、無限小の計算を形式化したときに二重数が果たす役割です。
...
円と双曲線が退化して2本の平行な直線になったものが二重数の「単位円」です。$e^{\varepsilon\theta}$を掛ける操作は、実部を保ったまま$\varepsilon$部を$\theta x$だけずらす、せん断（シアー）として働きます。回転角が「進んでも戻ってこない」この構造が、後で並進を表す鍵になります。
...
として埋め込みます。先頭の$1$は一見不自然ですが、同次座標の「重み」に相当し、この形が保たれることが変換の整合性を担保します。後で見るPGAでは、この埋め込みが自然に導かれます。

## 並進と回転

まず並進です。純虚四元数$t$に対して$T = 1 + \frac\varepsilon2 t$とおきます。$\overline t = -t$より$\overline T = 1 - \frac\varepsilon2 t$、$\overline{T^*} = 1 + \frac\varepsilon2 t = T$となることに注意して、$\overline{T^*}$で挟むと
...
&&&rem なぜ第3の共役が必要か
通常の四元数共役$\overline\sigma$で挟むと、並進が両側から逆符号で寄与して打ち消されてしまいます。
...
道具が揃ったので、出発点だった無限小三角形の計算に戻ります。球面三角法の記事で導いたマスター方程式を再掲します。[[tao-h]]

&&&fml 球面三角法のマスター方程式
球面三角形の辺$a,b,c$と内角$\alpha,\beta,\gamma$（頂点は時計回り）に対して

$$
e^{i(\pi-\alpha)/2} e^{kb/2} e^{i(\pi-\gamma)/2} e^{ka/2} e^{i(\pi-\beta)/2} e^{kc/2} = -1 \tag{3}
$$
&&&

$e^{k\cdot/2}$は辺に沿った移動、$e^{i\cdot/2}$は頂点での方向転換（外角）を表し、三角形を一周すると球面の向きが元に戻って$2\pi$回転分の$-1$が残る、というのがこの式の内容でした。

ここで辺の長さを$a \to \varepsilon a,\ b \to \varepsilon b,\ c \to \varepsilon c$と置き換えます。球面三角法の記事ではここでテイラー展開による1次近似を使いましたが、$\varepsilon^2 = 0$を代数規則として持つ二重四元数の中では、指数関数が**厳密に**閉じます。

$$
e^{k\varepsilon a/2} = 1 + \frac{k \varepsilon a}2
$$

そして右辺は、前節で見た「ベクトル$ka$だけの並進」そのものです。方向転換の因子$e^{i(\pi-\beta)/2}$は通常の回転子のまま残ります。つまり、辺の置き換え後のマスター方程式の左辺は、平面上で「進んで、曲がって、進んで……」を繰り返す動き（タートルグラフィックス）を剛体変換として合成したものになっています。

計算も球面のときと同様に進みます。$ik = -ki$から従う$k e^{i\varphi} = e^{-i\varphi/2} k\, e^{i\varphi/2}$を使って方向転換の因子をすべて左に集めると、$\varepsilon$同士の積は消えるため、剰余項なしで次の形になります。
...
= e^{i(3\pi - (\alpha+\beta+\gamma))/2}
   \left(1 + \frac{k\varepsilon}2 S \right) \tag{4}
$$

$$
S := b\, e^{i(\pi-\gamma)} e^{i(\pi-\beta)} + a\, e^{i(\pi-\beta)} + c
$$

球面三角法の記事の計算と見比べると、$O(\varepsilon^2)$と書かれていた剰余項が消えている以外は同一です。近似だった式が恒等式に昇格しました。

&&&thm 平面三角法のマスター方程式
(4)の左辺が$-1$に等しいことは、次の2条件が同時に成り立つことと同値です。

1. **内角の和** $\alpha + \beta + \gamma = \pi$（実部分の条件）
2. **辺ベクトルの閉合** $S = 0$（二重部分の条件）

すなわち、置き換え後のマスター方程式は平面三角形の成立条件を過不足なく表します。
&&&

&&&prf
(4)の右辺は$A(1 + \frac{k\varepsilon}2 S)$（$A := e^{i(3\pi - (\alpha+\beta+\gamma))/2}$）の形で、実部分が$A$、二重部分が$\frac12 A k S$です。これが$-1$になる条件は$A = -1$かつ$S = 0$です。内角が$0 < \alpha,\beta,\gamma < \pi$の範囲では$A = -1 \iff \alpha+\beta+\gamma = \pi$となります。
&&&

$S$は「辺$c$を進み、外角$\pi-\beta$だけ曲がって辺$a$を進み、外角$\pi-\gamma$だけ曲がって辺$b$を進む」という道のりの変位を複素数で表したものです。$S = 0$は出発点に戻ること、つまり三角形が閉じることを意味します。球面では6つの要素$a,b,c,\alpha,\beta,\gamma$への拘束が1本の式(3)にまとまっていましたが、平面ではそれが「角の条件」と「辺の条件」に分離する、と読むこともできます。

&&&ex 平面三角法の公式
閉合条件から平面三角法の公式が導けます。$\alpha+\beta+\gamma=\pi$を使うと$e^{i(\pi-\beta)} = -e^{-i\beta}$、$e^{i(2\pi-\beta-\gamma)} = e^{-i(\beta+\gamma)} = -e^{i\alpha}$なので

$$
S = c - a e^{-i\beta} - b e^{i\alpha} = 0
\iff c = a e^{-i\beta} + b e^{i\alpha}
$$

実部と虚部を取り出すと

$$
c = a\cos\beta + b\cos\alpha \quad \text{（第一余弦定理）}, \quad
a\sin\beta = b\sin\alpha \quad \text{（正弦定理）}
$$

さらに絶対値の2乗を取ると

$$
c^2 = |a e^{-i\beta} + b e^{i\alpha}|^2 = a^2 + b^2 + 2ab\cos(\alpha+\beta) = a^2 + b^2 - 2ab\cos\gamma
$$

と第二余弦定理も得られます。球面三角法では(3)から各公式を導くのに工夫が必要でしたが、平面では1本の閉合条件にすべてが入っています。
&&&

球面三角法の記事では「無限小の領域において、回転が並進として扱える」と述べるに留まりました。二重四元数はこの観察を代数として実装したものです。$\varepsilon$を「無限小」と読めば接空間（曲面に接する平面）の中の近似計算ですが、$\varepsilon^2=0$という規則だけを残して形式的に扱えば、接空間の縛りは外れ、平面全体・空間全体で厳密に成り立つ並進の代数になります。
...
$$
\exp\left(\frac{\hat\theta}2 L\right) = \cos\frac{\hat\theta}2 + L \sin\frac{\hat\theta}2 \tag{5}
$$
...
&&&prf
$m = x_0 \times l$とおき、ねじ運動を剛体変換の合成として組み立てて(5)と一致することを示します。軸を原点に平行移動して回転し、元に戻してから滑る、という合成です。並進$T(v) := 1 + \frac\varepsilon2 v$と回転子$r := e^{l\theta/2} = \cos\frac\theta2 + l\sin\frac\theta2$を使って
...
任意の単位二重四元数$\sigma = p + \varepsilon q$は、符号$\pm$を除いて(5)の形に書けます。したがって、任意の3次元剛体変換はあるねじ運動として実現できます。
&&&

&&&prf
$\pm$の符号を選んで$\mathrm{Re}(p) \geq 0$とします。$p \neq \pm1$のとき、実部分$p$は単位四元数なので$\theta = 2\operatorname{atan2}(|\mathrm{Im}(p)|, \mathrm{Re}(p))$、$l = \mathrm{Im}(p)/|\mathrm{Im}(p)|$と一意に分解できます。二重部分を(5)の展開と比較して
...
&&&rem
2つの姿勢の間の補間は、回転子では球面線形補間 (SLERP) として知られますが、二重四元数では(5)の二重角$\hat\theta$を等分するねじ運動の補間 (ScLERP) に拡張され、回転と並進が同一のねじ軸上で同時に補間されます。回転子の補間は別の記事で扱う予定です。
&&&
...
冒頭で見た2次元実代数の三分法（$-1$: 複素数、$+1$: 分解型複素数、$0$: 二重数）が、偶部分代数の分類としてそのまま再登場しています。$\omega^2 = +1$の場合に冪等元$\frac{1\pm\omega}2$による直和分解$\mathbb H \oplus \mathbb H$が生じることは4次元回転の記事で見た通りです。$\omega^2 = 0$の場合は直和に分解する代わりに、冪零なイデアル$\varepsilon\mathbb H$（並進部分）が現れます。回転群$\mathrm{Spin}(4)$を担った代数と剛体変換を担う代数が、擬スカラーの平方ひとつを付け替えた隣人どうしだったわけです。
...
- 剛体変換は$\sigma = (1 + \frac\varepsilon2 t) r$として表され、点$1 + \varepsilon x$を第3の共役$\overline{\sigma^*}$で挟むことで「回転＋並進」が実現します。単位二重四元数は$\mathrm{SE}(3)$を二重被覆します。
- 球面三角法のマスター方程式は、辺を$\varepsilon$倍すると剰余項なしで平面三角形の成立条件（内角の和と辺の閉合）に厳密に変換され、そこから平面の余弦定理・正弦定理が導かれます。
