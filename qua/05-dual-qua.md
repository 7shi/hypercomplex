...
生成元$e_1, e_2, e_3$（$e_a^2 = +1$）と$e_0$（$e_0^2 = 0$）が互いに反交換するとして生成される$2^4 = 16$次元の代数を$C\ell_{3,0,1}(\mathbb R)$と書きます。符号数の3つ組$(p,q,r) = (3,0,1)$は、平方が$+1, -1, 0$となる生成元の個数を表します。非退化な$C\ell_{p,q}(\mathbb R)$は$r=0$の場合、すなわち$C\ell_{p,q,0}(\mathbb R)$の略記です。
&&&

偶数個の生成元の積が張る偶部分代数$C\ell^+_{3,0,1}(\mathbb R)$（4次元回転の記事の$C\ell^0$と同じものです）は、スカラー$1$、6つの2ベクトル$e_ae_b$、擬スカラー$e_0e_1e_2e_3$の8次元です。これが二重四元数と同型になります。
...
対応表の8元は$C\ell^+_{3,0,1}(\mathbb R)$の基底なので、対応は線形同型である。積は生成元$i, j, k, \varepsilon$の間の関係式だけで決まるから、上の確認によって積も保たれ、代数の同型が得られる（すべての基底の組み合わせは検証コードでも確認した）。
&&&
...
- 二重四元数は退化クリフォード代数の偶部分代数$C\ell^+_{3,0,1}(\mathbb R)$と同型であり、$e_1, e_2, e_3$に加える第4の生成元の平方$-1, +1, 0$に応じて双四元数・分解型双四元数・二重四元数が現れるという三分法が成り立ちます。これはPGA・CGAへの入り口です。
