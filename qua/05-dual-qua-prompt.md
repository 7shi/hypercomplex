...
- 数式・数学的主張の正しさ。特に次の導出を検算してください。
    ...
    - 剛体変換のサンドイッチ公式 $\sigma(1+\varepsilon x)\overline\sigma^* = 1 + \varepsilon(rxr^* + t)$ の展開と、$\sigma^*$ で挟むと並進が消えるという対比計算。
    - 合成則 (2) と、$\mathrm{SE}(3)$ の合成規則 $x \mapsto r_2(r_1xr_1^*+t_1)r_2^*+t_2$ の一致。
...
- 説明の分かりやすさ、論理の飛躍の有無。特に、基点を $i$ から $1$ に取り替える箇所で、なぜ通常の共役 $\sigma^*$ では駄目で第3の共役 $\overline\sigma^*$ が要るのかという動機づけが、読者の側から追える順序になっているか。
- 用語・記法の一貫性。参照記事との照合を含めて確認してください。
    ...
    - 内積 $\langle q,r \rangle := \mathrm{Re}(qr^*)$、共役 $*$（四元数）・上線（二重数）の使い分けが、参照記事の記法と衝突していないか。
