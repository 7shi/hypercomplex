...
&&&rem 必要十分条件としては成立しない
行列式の条件から、固定した$\rho$に対する$vu=\rho uv$が従うという部分は、そのままでは成立しません。$u$と$v$を入れ替えても行列式$\det(zI+yu+xv)=z^3+y^3+x^3$は対称性から保たれますが、交換関係の係数は$\rho$から$\rho^{-1}=\rho^2$に変わります。どちらの向きの原始$3$乗根が現れるかは行列式では決まらないため、この必要十分性を固定した$\rho$についての現代的な定理としてそのまま採用することはできません。歴史的な主張として読み、以下では具体的な行列から交換関係を直接確認します。
&&&
