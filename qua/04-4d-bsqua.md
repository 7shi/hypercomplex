...
&&&rem 回転の規約
本記事では、クリフォード代数による回転を$v'=r^{-1}vr$の向きで表します。$v'=rvr^{-1}$と逆向きに書く流儀もあり、その場合は回転子の積に対する作用の合成順序が入れ替わります。
&&&

# 左右作用と$M_4(\mathbb R)$

四元数$q$を基底$(1,i,j,k)$に関する実4次元の列ベクトルとみなし、虚数単位$u=i,j,k$を左右から掛ける操作を$L_u,R_u$とします。

$$
L_u\,q=uq,\quad R_u\,q=qu
$$

$q$について線形なので、$L_u,R_u$は$4\times4$実行列で表されます。結合律から積の規則が決まり、左作用と右作用は互いに可換です。
...
$$
\mathbb H\otimes_{\mathbb R}\mathbb H\longrightarrow M_4(\mathbb R),\quad
u\otimes v\longmapsto L_uR_{v^*}
$$
&&&
...
ここで重要なのは、$M_4(\mathbb R)$の元が4次元ベクトルに直接作用する行列だという点です。単位四元数による左右からの挟み込みは、1つの行列にまとまります。

$$
q'=r_L\,q\,r_R=L_{r_L}R_{r_R}\,q
$$

ノルムの乗法性より$|r_L\,q\,r_R|=|q|$なので、$L_{r_L}R_{r_R}$は長さを保つ直交行列、実際には行列式$1$の$\mathrm{SO}(4)$の回転行列そのものです（単位四元数は$1$と連続につながるため行列式は$+1$）。逆に$\mathrm{SO}(4)$のすべての元がこの形で書けることが知られています。つまり、左右からの作用は$\mathrm{SO}(4)$を構成します。

この構成を群の言葉で整理します。単位四元数のなす群は$\mathrm{SU}(2)$と同型です。ただし、対$(r_L,r_R)$をそのまま組にしたのでは、直積群からの準同型になりません。右作用は積の順序が反転するため

$$
(L_aR_b)(L_cR_d)=L_{ac}R_{db}
$$

と、第2成分だけ積が逆順になるからです。そこで$a=r_L$、$b=r_R^{-1}$と置き直します。

$$
F(a,b)=L_aR_{b^{-1}},\quad q'=a\,q\,b^{-1}
$$

こうすれば$F$は直積群$\mathrm{SU}(2)\times\mathrm{SU}(2)$から$\mathrm{SO}(4)$への全射準同型となり、その核は$(1,1)$と$(-1,-1)$の2元です。準同型定理から、$\mathrm{SO}(4)$は直積群を核で割った商として表されます。
...
&&&prf
$F(a,b)=L_aR_{b^{-1}}$の核を求める。核の元は任意の四元数$q$に対して$a\,q\,b^{-1}=q$を満たす。$q=1$と取れば$ab^{-1}=1$、すなわち$b=a$である。これを戻すと任意の$q$で$a\,q\,a^{-1}=q$、つまり$a$はすべての四元数と可換である。$a=a_0+a_1i+a_2j+a_3k$とおくと、$i$との可換性から$a_2=a_3=0$、$j$との可換性から$a_1=0$が従い、$a$は実数となる。単位四元数であることから$a=\pm1$、対応して$(a,b)=(1,1),(-1,-1)$である。逆にこの2元が恒等変換を与えることは明らかであるため、核はちょうどこの2元である。
&&&

テンソル積との関係は、群そのものの記法としてではなく、写像の像として述べると明確です。実代数の同型

$$
\mathbb H\otimes_{\mathbb R}\mathbb H\longrightarrow M_4(\mathbb R),\quad
a\otimes b\longmapsto L_aR_{b^*}
$$

のもとで、単位四元数の対から作られる単純テンソル$a\otimes b$の全体は、$M_4(\mathbb R)$の中で上の商群と同型な部分群をなします。テンソル積では$(-a)\otimes(-b)=a\otimes b$と符号が相殺するため、直積群そのものではなく、核で割った商が像として現れるのです。像の回転行列だけを見ると、2つの持ち上げ$(a,b)$と$(-a,-b)$を区別する符号は失われます。符号を区別する直積群$\mathrm{SU}(2)\times\mathrm{SU}(2)$が二重被覆$\operatorname{Spin}(4)$であり、$M_4(\mathbb R)$の中に住んでいるのは、その商である$\mathrm{SO}(4)$です。

符号の情報がこの商で失われている以上、$q$への作用は標準的な意味でのスピノルでもありません。スピノルは、片側だけの作用$\psi\mapsto r\psi$が半角で効くために$2\pi$回転（$r\to-r$）で符号が反転するという性質で特徴づけられますが、$(r_L,r_R)\to(-r_L,-r_R)$としても$q$への作用が変わらない以上、この性質を満たしません。
...
$$
したがって$i$を$u$に置き換えるだけで、$(1,u)$平面とそれに直交する平面の二重回転が同様に得られ、回転面の向き（3次元的な回転軸の向き）を自由に選べます。ただし、この方法で選べるのは実軸を含む平面$(1,u)$とその直交補平面という組に限られ、一般の2平面分解がすべてこの形で書けるわけではありません。一般の回転では、後述するように左右の軸を独立に取ります。
&&&
...
ここでベクトル$v=q_0e_1+q_1e_2+q_2e_3+q_3e_4$は四元数$q=q_0+q_1i+q_2j+q_3k$と同一視します。回転子は、6成分の2ベクトル$B$の指数関数として与えられます。
...
2ベクトルの2乗は、一般にはスカラーとグレード4の和になります。擬スカラーを$\Omega=e_1e_2e_3e_4$と書くと、対をなす基底の積がグレード4を生み

$$
B^2=-(b_{12}^2+b_{13}^2+b_{14}^2+b_{43}^2+b_{24}^2+b_{32}^2)-2(b_{12}b_{43}+b_{13}b_{24}+b_{14}b_{32})\,\Omega
$$

となります。

&&&ex 単純回転
$p^2=-1$を満たす2ベクトル$p$を**単純単位2ベクトル**と呼びます。上式より、これは対をなす係数の積の和が消え、かつ全体が規格化された場合です。
$$
p_{12}p_{43}+p_{13}p_{24}+p_{14}p_{32}=0,\quad
p_{12}^2+p_{13}^2+p_{14}^2+p_{43}^2+p_{24}^2+p_{32}^2=1
$$
このとき$p$は1つの平面を表します。$B=\theta p$と取れば指数関数が展開でき、回転子は三角関数で表されます。
$$
...
同じ回転が四元数側では$q'=r_L\,q\,r_R$と表されます。回転子の指数を与える2ベクトルの6成分が、$3+3$成分の$(r_L,r_R)$にどう対応するのかが問題となります。
...
単純回転の回転子はスカラー（グレード0）と2ベクトル（グレード2）の線形結合で、その積からはグレード4の$\Omega=e_1e_2e_3e_4$も現れます。一般の4次元回転子は、最初からグレード$0,2,4$を持ち得ます。偶数グレードの元は積について閉じており、**偶部分代数**をなします。偶部分代数は、特定の生成元を含む2ベクトルを新たな生成元と見なすことで、生成元が1つ少ない（次元が1つ下がった）クリフォード代数と同型になります。新たな生成元の2乗は$(e_me_n)^2=-e_m^2e_n^2$で決まります。
...
3次元回転が記述される$C\ell_{0,3}(\mathbb R)$が、1つ上の4次元では偶部分代数として再登場しています。$C\ell_{0,3}(\mathbb R)$は「四元数と、それに可換な体積要素$\Omega$」という形をしており、4次元では$\Omega^2=e_1^2e_2^2e_3^2e_4^2$となるため、どちらの計量でも$\Omega^2=+1$です。これが**分解型双四元数**$\mathbb C'\otimes\mathbb H$であり、分解型複素数と同様に冪等元$\dfrac{1\pm\Omega}2$によって$\mathbb H\oplus\mathbb H$へ直和分解できます。後で見るように、この2つの四元数成分がちょうど左右の回転子$(r_L,r_R)$を受け止めます。対は符号ごと保持されるため（$(-r_L,-r_R)\ne(r_L,r_R)$）、単位四元数の対がなすのは商$\mathrm{SO}(4)$ではなく直積$\mathrm{SU}(2)\times\mathrm{SU}(2)\cong\operatorname{Spin}(4)$です。回転行列そのものを構成する$M_4(\mathbb R)$と、対を符号ごと保持する$\mathbb H\oplus\mathbb H$という、2つの表現がここで分岐します。
...
行き先の$\{i,j,k,\omega\}$が生成する代数が分解型双四元数で、任意の元は$X'+\omega X''$（$X',X''\in\mathbb H$）と書けます。$\Omega$を掛けると対をなす2ベクトルが入れ替わるため、$e_1$を含む2ベクトルは$\omega i,\omega j,\omega k$へ移ります。
...
まず冪等元を作ります。$\omega^2=1$なので、$\varepsilon=\dfrac{1+\omega}2$は冪等元です。冪等性に加えて、$\varepsilon$によって$\omega$が$1$に移されることも確かめられます。
...
もう1つの冪等元$1-\varepsilon=\dfrac{1-\omega}2$も同様に冪等で、こちらでは$\omega$が$-1$に移されます。
...
&&&rem
「$\omega$を掛ける」操作は、分解型双四元数を四元数係数の2次元線形空間（基底$1,\omega$）とみなしたときの線形写像です。上の2式は、$\varepsilon$と$1-\varepsilon$がこの線形写像の固有ベクトルであり、固有値がそれぞれ$1$と$-1$であることを表しています。$\omega$はすべての元と可換なため、固有ベクトルを四元数倍しても固有値は変わりません。固有値$1$の固有空間は$\varepsilon$の四元数倍全体$\varepsilon\,\mathbb H$、固有値$-1$の固有空間は$(1-\varepsilon)\,\mathbb H$で、それぞれ四元数係数で1次元です。
&&&
...
つまり$X$は、和成分$X'+X''$を$\varepsilon$に、差成分$X'-X''$を$1-\varepsilon$に掛けた形に分解されます。$\varepsilon,\ 1-\varepsilon$を対角行列$\operatorname{diag}(1,0),\ \operatorname{diag}(0,1)$へ対応させて四元数係数の対角行列でこれを表せば、$\varepsilon,\ 1-\varepsilon$は文字通りの**射影行列**になります。
...
もう一方の成分（差$X'-X''$）も、同じ$T$で読み出せます。そのために$\omega$の符号だけを反転させる操作$\dagger$を導入します。
...
行列表現では2つの対角成分の入れ替え$\operatorname{diag}(A,B)\mapsto\operatorname{diag}(B,A)$なので、差成分は$T(X^{\dagger})=X'-X''$として得られます。

## 純虚四元数への変換

2ベクトル$B$を$\varphi$で分解型双四元数へ移してから$T$を適用すると、和成分$U_R$が得られます。

$$
\begin{aligned}
U_R
&=T(\varphi(B)) \\
&=T(b_{12}\omega i+b_{13}\omega j+b_{14}\omega k+b_{43}i+b_{24}j+b_{32}k) \\
&=b_{12}i+b_{13}j+b_{14}k+b_{43}i+b_{24}j+b_{32}k \\
&=(b_{12}+b_{43})i+(b_{13}+b_{24})j+(b_{14}+b_{32})k
\end{aligned}
$$

$\omega$共役で差成分に切り替えれば、$U_L$も得られます。

$$
\begin{aligned}
U_L
&=-T(\varphi(B)^{\dagger}) \\
&=-T(-\omega(b_{12}i+b_{13}j+b_{14}k)+b_{43}i+b_{24}j+b_{32}k) \\
&=b_{12}i+b_{13}j+b_{14}k-b_{43}i-b_{24}j-b_{32}k \\
&=(b_{12}-b_{43})i+(b_{13}-b_{24})j+(b_{14}-b_{32})k
\end{aligned}
$$

$U_L,U_R$は純虚四元数で、その向きが回転軸、その大きさが回転量を表します。単位とは限らないため、軸と角を分けずに、軸方向と回転量をまとめた純虚四元数として扱います。これによって回転子を表します。

$$
r_L=\exp\left(\frac{U_L}2\right),\quad r_R=\exp\left(\frac{U_R}2\right)
$$
...
よって、回転は次のように分解型双四元数経由で四元数に移せます。

$$
\begin{aligned}
r^{-1}vr
&=\exp\left(-\frac B2\right)v\exp\left(\frac B2\right) \\
&\mapsto
...
\begin{alignedat}{3}
q'&=r_L\,q\,r_R \\
r_L&=\exp\left(\frac{U_L}2\right),&\quad
U_L&=-T(\varphi(B)^{\dagger})&&=(b_{12}-b_{43})i+(b_{13}-b_{24})j+(b_{14}-b_{32})k \\
r_R&=\exp\left(\frac{U_R}2\right),&\quad
U_R&=T(\varphi(B))&&=(b_{12}+b_{43})i+(b_{13}+b_{24})j+(b_{14}+b_{32})k
\end{alignedat}
$$
&&&

成分を見ると、対をなす成分の和が$U_R$、差が$U_L$となり、二重回転の節で回転角に見た「和と差への組み替え」が、2ベクトルの6成分全体に広がった形です。

自由度の勘定も成分のレベルで裏付けられます。和$(b_{12}+b_{43},\ b_{13}+b_{24},\ b_{14}+b_{32})$と差$(b_{12}-b_{43},\ b_{13}-b_{24},\ b_{14}-b_{32})$から元の6成分は復元できるため、この組み替えで情報は失われません。和と差の各3成分は互いに独立に選べ、そのまま左右の回転子$r_L,r_R$の3自由度（軸の向き2＋回転量1）に対応します。$3+3=6$が$\mathrm{SO}(4)$の次元と一致するのは、この可逆な組み替えの反映です。

&&&prf $v'=r^{-1}vr$と$q'=r_L\,q\,r_R$が同じ回転を表すこと
本文で述べたのは、回転を構成する部品（2ベクトル$B$とベクトル$v$）がそれぞれ$T\circ\varphi$で四元数に移せるということであり、部品ごとの対応だけでは、移した部品を組み合わせた式が元と同じ回転を表すことはまだ保証されない。示すべきことは、ベクトルと四元数の同一視$v\leftrightarrow q$（本文の計算で確認したとおり$q=T(\varphi(e_1v))$）のもとで、回転後のベクトル$v'=r^{-1}vr$に対応する四元数が$r_L\,q\,r_R$に一致することである。これは「等傾回転の合成」の節の結果に帰着できる。

回転子の指数の肩を$1\pm\Omega$で振り分ける。

$$
\frac B2=\frac14(1-\Omega)B+\frac14(1+\Omega)B
$$
...
$$
\begin{alignedat}{2}
(1-\Omega)B&=(1-\Omega)b_L,&\quad
b_L&=(b_{12}-b_{43})e_1e_2+(b_{13}-b_{24})e_1e_3+(b_{14}-b_{32})e_1e_4 \\
(1+\Omega)B&=(1+\Omega)b_R,&\quad
b_R&=(b_{12}+b_{43})e_1e_2+(b_{13}+b_{24})e_1e_3+(b_{14}+b_{32})e_1e_4
\end{alignedat}
$$
...
第1因子は、$b_L=|U_L|\,\hat b$（$\hat b^2=-1$）と規格化すれば、軸$\hat b$・角$\dfrac{|U_L|}2$の左作用の等傾回転子$\exp\left(\dfrac{|U_L|}4(1-\Omega)\hat b\right)$である（$U_L=0$なら恒等）。「等傾回転の合成」の節より、その作用は四元数側では左作用となる。

$$
q\mapsto\exp\left(\frac{|U_L|}2\,\hat u\right)q=\exp\left(\frac{U_L}2\right)q=r_L\,q
$$

ここで$\hat u$は$\hat b$に対応する軸で、$|U_L|\hat u=U_L$である。同様に第2因子は右作用$q\mapsto q\exp\left(\frac{U_R}2\right)=q\,r_R$として作用する。回転子$r$は2つの因子の積だから、回転も2つの作用の合成となる。

$$
v'=r^{-1}vr\quad\leftrightarrow\quad q'=r_L\,q\,r_R
$$

これで本文に掲げた式が示された。$T\circ\varphi$による表示とは次のようにつながる。本文の計算のとおり$U_R=T(\varphi(B))$、$U_L=-T(\varphi(B)^{\dagger})$であり、$\varphi,T,\dagger$はいずれも準同型で指数関数と可換だから

$$
r_R=\exp\left(\frac12T(\varphi(B))\right)=T(\varphi(r)),\quad
r_L=\exp\left(-\frac12T(\varphi(B)^{\dagger})\right)=T(\varphi(r)^{\dagger})^{-1}
$$

が成り立つ。生成子$U_L$には負号、回転子$r_L$には逆元が付くことに注意する。
&&&

同じ結論は、$e_1$による共役が$\varphi$を通して$\omega$共役に対応することを使えば、回転子について一度に導けます。
...
&&&prf
両辺とも$X$について線形なので、偶部分代数の基底で確かめれば十分である。$e_1^2=1$で、$e_1$は$e_1$を含む2ベクトルと反交換し、$e_1$を含まない2ベクトルとは可換であり、$\Omega$とは反交換するから

$$
e_1(e_1e_a)e_1=-e_1e_a,\quad
e_1(e_ae_b)e_1=e_ae_b\ (a,b\ne1),\quad
e_1\Omega e_1=-\Omega
$$

となる。$\varphi$は$e_1$を含む2ベクトルを$\omega i,\omega j,\omega k$へ、含まない2ベクトルを$i,j,k$へ、$\Omega$を$\omega$へ移すので、符号が反転するのはちょうど$\omega$を含む成分である。スカラーは両辺とも不変であるから、これは$\omega$の符号だけを反転させる操作$\dagger$に一致する。
&&&

&&&prf 射影による回転の対応
$q=T(\varphi(e_1v))$と置く。$e_1^2=1$より
...
となる。すなわち$r_L=T(\varphi(r)^{\dagger})^{-1}$、$r_R=T(\varphi(r))$であり、$r$が一般の回転子のまま結論が得られる。
&&&

&&&rem 左右の回転子と直和分解
$r_R=T(\varphi(r))$と$r_L=T(\varphi(r)^{\dagger})^{-1}=T(\varphi(r)^{\dagger})^*$が示すように、偶部分代数$\mathbb H\oplus\mathbb H$の2つの直和成分は、共役と逆元を介して左右の回転子に対応します。
&&&
...
&&&rem 自己双対分解
和と差への組み替えは、2ベクトルの言葉では対をなす基底の和と差$e_1e_2\pm e_4e_3$などへの組み替えです。$\Omega$を掛けると対をなす2ベクトルが入れ替わるため（$\Omega\,e_1e_2=e_4e_3$）、和は不変で、差は符号が反転します。

向きを$(e_1,e_2,e_3,e_4)$に取った通常のホッジ双対は$*(e_1\wedge e_2)=e_3\wedge e_4$ですが、クリフォード積では$\Omega\,e_1e_2=e_4e_3=-e_3e_4$なので、2ベクトル上では$*B=-\Omega B$という関係になります。したがって、$\Omega$倍で不変な和$e_1e_2+e_4e_3$は**反自己双対**、符号が反転する差$e_1e_2-e_4e_3$が**自己双対**です。この直和分解はリー代数の分解$\mathfrak{so}(4)\cong\mathfrak{su}(2)\oplus\mathfrak{su}(2)$（$\operatorname{Spin}(4)\cong\mathrm{SU}(2)\times\mathrm{SU}(2)$のリー代数版）そのものです。
&&&
