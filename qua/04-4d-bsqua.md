...
$$
\ L_iR_i \mapsto e_1,\ L_jR_i\mapsto e_2,\ L_kR_i\mapsto e_3,\ R_j\mapsto e_4
$$
に取れば、2乗は
$$
...
$$
p=p_{12}e_1e_2+p_{13}e_1e_3+p_{14}e_1e_4+p_{43}e_4e_3+p_{24}e_2e_4+p_{32}e_3e_2 \quad (p^2=-1)
$$
...
同じ回転が四元数側では$q'=r_L\,q\,r_R$と表されます。6成分の回転子が$3+3$成分の$(r_L,r_R)$にどう対応するのかが問題となります。
...
| グレード4 | $-e_1e_2e_3e_4$                            | グレード3 | $f_1f_2f_3$                 | $\omega$ | $+1$  |
...
$C\ell_{4,0}(\mathbb R)$全体（16次元）は分解型双四元数（8次元）と同型ではないため、変換は偶部分代数からの同型写像$\varphi$として明示し、暗黙の同一視は避けます。上表のとおり、$\varphi$はグレード2の基底を四元数の基底$i,j,k$へ、擬スカラー$e_1e_2e_3e_4$を分解型双四元数の$\omega$へ移します。
...
行き先の$\{i,j,k,\omega\}$が生成する代数が分解型双四元数で、任意の元は$X'+\omega X''$（$X',X''\in\mathbb H$）と書けます。$\omega$を掛けるとグレード1と2が入れ替わるため、$e_1$を含む2ベクトルは$\omega i,\omega j,\omega k$へ移ります。
...
回転面$p$を$C\ell_{4,0}(\mathbb R)$のグレード2の元で定義します。

$$
p = p_{12}e_1e_2+p_{13}e_1e_3+p_{14}e_1e_4+p_{43}e_4e_3+p_{24}e_2e_4+p_{32}e_3e_2 \quad (p^2=-1)
$$

$\varphi$で分解型双四元数へ移してから$T$を適用すると、回転軸$q_R$が得られます。

$$
\begin{aligned}
q_R
&=T(\varphi(p)) \\
&=T(p_{12}\omega i+p_{13}\omega j+p_{14}\omega k+p_{43}i+p_{24}j+p_{32}k) \\
&=p_{12}i+p_{13}j+p_{14}k+p_{43}i+p_{24}j+p_{32}k \\
&=(p_{12}+p_{43})i+(p_{13}+p_{24})j+(p_{14}+p_{32})k
\end{aligned}
$$

$\omega$共役で差成分に切り替えれば、回転軸$q_L$にも変換できます。
