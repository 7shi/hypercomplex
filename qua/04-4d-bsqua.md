...
&&&rem 回転の規約
本記事では、クリフォード代数による回転を$v'=r^{-1}vr$の向きで表します。この規約では、回転子の積に対して作用の合成順序が逆転します。
$$
\rho_r(v)=r^{-1}vr
\quad\Longrightarrow\quad
\rho_{rs}=\rho_s\circ\rho_r
$$
$v'=rvr^{-1}$と逆向きに書く流儀もあり、その場合は合成順序がそのまま保たれます。
&&&
...
この商を経由する以上、$q\in\mathbb H\simeq\mathbb R^4$への作用は$\mathrm{SO}(4)$の標準ベクトル表現であって、スピノル表現ではありません。$\operatorname{Spin}(4)$の表現として見ると、被覆の核にある中心元$(-1,-1)$が恒等変換として作用します。一方、スピノル表現ではこの中心元が$-I$として作用し、符号の情報が保たれます。
...
&&&rem 半スピノルの次元
最低8次元というのは、群全体に忠実な実表現についての主張です。個々の半スピノル表現は実4次元で存在しますが、直積の片方の因子しか作用しないため、群全体には忠実ではありません。
&&&
...
$$
と動きます。つまり左作用$q\mapsto e^{\alpha i}q$は、$(1,i)$平面と$(j,k)$平面を共に角度$\alpha$だけ回す回転です。このように、直交する2平面で回転角の大きさが等しい回転を**等傾回転**と呼びます。向き付き角度の符号は各平面の向きの取り方に依存するため、以下では符号も含めて明示します。虚数単位$i$を右から掛けると、基底は
$$
(1,i,j,k) \mapsto (i,-1,-k,j)
$$
...
同じ回転が四元数側では$q'=r_L\,q\,r_R$と表されます。回転子の指数を与える2ベクトルの6成分が、左右の回転子$(r_L,r_R)$の各3自由度にどう対応するのかが問題となります。
...
単純回転の回転子はスカラー（グレード0）と2ベクトル（グレード2）の線形結合で、その積からはグレード4の$\Omega=e_1e_2e_3e_4$も現れます。一般の4次元回転子は、最初からグレード$0,2,4$を持ち得ます。偶数グレードの元は積について閉じており、**偶部分代数**をなします。偶部分代数は、特定の生成元を含む2ベクトルを新たな生成元と見なすことで、生成元が1つ少ない（生成ベクトル空間の次元が1つ下がった）クリフォード代数と同型になります。新たな生成元の2乗は$(e_me_n)^2=-e_m^2e_n^2$で決まります。
...
回転させるベクトル$v$はグレード1で、そのままでは偶部分代数に入らないため、$\varphi$で移せません。そこで$e_1$を掛けます。$e_1^2=1$となってグレード0（スカラー）になり、それ以外はグレード2（2ベクトル）になるため、偶部分代数に収まります。これによってベクトルも分解型双四元数経由で四元数に移せます。
...
$$
q'=r_L\,q\,r_R,\quad q=T(\varphi(e_1v))
$$
$$
\begin{alignedat}{3}
...
&&&rem 可逆なのは生成子の線形変換
これは生成子の成分の間の線形変換についての主張です。指数写像には周期性があるため、回転子や回転から生成子を一意に復元できるわけではありません。例えば
$$
B=0,\qquad B=4\pi e_1e_2
$$
は、どちらも$r=\exp(B/2)=1$を与えます。軸と回転量による自由度の勘定も、大域的に一意な座標表示を意味するものではありません。
&&&

証明を2通り与えます。まず、$e_1$による共役が$\varphi$を通して$\omega$共役に対応することを使うと、回転子$r$を分解せずに直接示せます。
...
となる。すなわち$r_L=T(\varphi(r)^{\dagger})^{-1}$、$r_R=T(\varphi(r))$であり、$r$が一般の回転子のまま結論が得られる。$r=\exp(B/2)$と置けば、$\varphi,T,\dagger$はいずれも準同型で指数関数と可換だから

$$
r_R=\exp\left(\frac12T(\varphi(B))\right),\quad
r_L=\exp\left(-\frac12T(\varphi(B)^{\dagger})\right)
$$

となり、指数表示$r_L=\exp(U_L/2)$、$r_R=\exp(U_R/2)$が従う。生成子$U_L$には負号、回転子$r_L$には逆元が付くことに注意する。
&&&

もう1つは、回転子を左右の等傾回転子の積に分解する証明です。計算は長くなりますが、左右の作用がどの平面をどちら向きに回しているかが見える形になります。
...
(s_Lr_L,\ r_Rs_R)
$$
と、左側だけ順序が逆転します。これは冒頭で断った規約$\rho_{rs}=\rho_s\circ\rho_r$の現れです。
&&&
