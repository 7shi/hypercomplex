...
左右の積$L_uR_v$（$u,v\in\{1,i,j,k\}$）は16個あり、一次独立で$4\times4$実行列全体$M_4(\mathbb R)$を張ります。これはテンソル積の同型$\mathbb H\otimes\mathbb H\cong M_4(\mathbb R)$の具体的な実現であり（以下、テンソル積はすべて$\mathbb R$上で取ります）、2つのテンソル因子が左作用と右作用として現れています。左右から挟む回転$q'=r_L\,q\,r_R=L_{r_L}R_{r_R}\,q$は、この行列環の中に住んでいます。
...
$\operatorname{Spin}(4)$の群構造を保ち、異なる群要素を区別する忠実な実行列表現は、$4\times4$実行列では実現できません。$\operatorname{Spin}(4)\cong\mathrm{SU}(2)\times\mathrm{SU}(2)$の忠実な実表現は最低8次元であることが知られており、その置き場所が後で見る偶部分代数$\mathbb H\oplus\mathbb H$です。ここで$\mathbb H\oplus\mathbb H$の代数としての実次元$8$と、忠実な実表現の表現空間の次元$8$は本来別の概念ですが、$\mathbb H\oplus\mathbb H$が自身に左から作用することで、実8次元の忠実な表現が得られます。
...
$r_L,r_R$の$3+3$自由度が$\mathrm{SO}(4)$の6自由度をどう覆うのかは、クリフォード代数に進む前に、四元数の計算だけで確認できます。鍵となる事実は、4次元の任意の回転が、直交基底を適切に取れば、互いに直交する2つの平面がそれぞれ独立な角度で回る**二重回転**の標準形にできることです。
...
$U_L,U_R$は純虚四元数で、左右それぞれの等傾回転の軸方向と角度パラメータをまとめたものです。単位とは限らないため、軸と角を分けずに扱います。$U_L\ne0$なら、左作用の軸は$U_L/|U_L|$、等傾回転の角度パラメータは$|U_L|/2$で、右作用も同様です。この「軸」は3次元的な回転軸に相当するもので、4次元回転全体の固定軸ではないことに注意します。これによって回転子を表します。
...
自由度の勘定も成分のレベルで裏付けられます。和$(b_{12}+b_{43},\ b_{13}+b_{24},\ b_{14}+b_{32})$と差$(b_{12}-b_{43},\ b_{13}-b_{24},\ b_{14}-b_{32})$から元の6成分は復元できるため、この組み替えで情報は失われません。和と差の各3成分は互いに独立に選べ、そのまま左右の回転子$r_L,r_R$の3自由度（軸の向き2＋角度パラメータ1）に対応します。$3+3=6$が$\mathrm{SO}(4)$の次元と一致するのは、この可逆な組み替えの反映です。
