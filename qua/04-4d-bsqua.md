...
# 左右作用と$\operatorname{Cl}_{3,1}(\mathbb R)$
...
この$M_4(\mathbb R)$は、それ自身が1つのクリフォード代数です。たとえば

$$
e_1\cong L_iR_i,\quad e_2\cong L_jR_i,\quad e_3\cong L_kR_i,\quad e_4\cong R_j
$$

と対応させれば、2乗は$(+1,+1,+1,-1)$で互いに反交換し、ミンコフスキー計量のクリフォード代数$\operatorname{Cl}_{3,1}(\mathbb R)$を生成します：

$$
\operatorname{Cl}_{3,1}(\mathbb R)\cong M_4(\mathbb R)\cong\mathbb H\otimes\mathbb H
$$

16個の基底$L_uR_v$は、符号を除いてグレード別の基底と1対1に対応します：
...
このことは、左右の対が$\operatorname{Cl}_{3,1}(\mathbb R)$のグレード構造と噛み合わない理由の説明になります。クリフォード代数の標準的な構成では、回転子は偶部分代数に住む元で、同じ代数のグレード1のベクトルを共役$r^{-1}vr$で挟みます。ところが$\operatorname{Cl}_{3,1}(\mathbb R)\cong M_4(\mathbb R)$において、回される四元数はグレード1のベクトルではなく、行列が片側から作用する列ベクトル——すなわちこの代数の**スピノル**です。実際、グレード表を見ると、左四元数$L_i,L_j,L_k$はグレード2に収まる一方、右四元数は$R_j$がグレード1、$R_i$がグレード3、$R_k$がグレード4と散らばり、左右対称なはずの対が非対称に扱われています。$\operatorname{Cl}_{3,1}(\mathbb R)$のグレード構造が適合するのは、ミンコフスキー計量の回転（ローレンツ変換）を回転子で表す枠組みです。$L_{r_L}R_{r_R}$はその枠組みの外で、同じ行列環にたまたま同居している$\mathrm{SO}(4)$の回転行列だと解釈できます。

&&&rem $\operatorname{Spin}(4)$の置き場所
...
$$
\mathbb H\otimes\mathbb H\cong M_4(\mathbb R)\cong\operatorname{Cl}_{3,1}(\mathbb R),\quad
...
四元数の左右から挟む作用は$\mathbb H\otimes\mathbb H\cong M_4(\mathbb R)\cong\operatorname{Cl}_{3,1}(\mathbb R)$を張り、$\mathrm{SO}(4)$の回転行列そのものを直接構成します。テンソル積では対の符号が相殺するため、そこに現れるのは直積$\operatorname{Spin}(4)\cong\mathrm{SU}(2)\times\mathrm{SU}(2)$ではなく商$\mathrm{SO}(4)\cong\mathrm{SU}(2)\otimes\mathrm{SU}(2)$であり、回される四元数もこの代数ではグレード1のベクトルではなくスピノルです。符号を区別する$\operatorname{Spin}(4)$の置き場所は、偶部分代数の階梯の上にあります。3次元回転では偶部分代数$\operatorname{Cl}_{0,2}(\mathbb R)\cong\mathbb H$に$\operatorname{Spin}(3)\cong\mathrm{SU}(2)$が住むように、4次元回転では偶部分代数$\operatorname{Cl}_{0,3}(\mathbb R)\cong\mathbb H\oplus\mathbb H$（分解型双四元数）に$\operatorname{Spin}(4)$が対$(r_L,r_R)$として住みます。
