...
5. クリフォード代数$\operatorname{Cl}_{n,0}(\mathbb{R})$（非可換、結合的）：n次元ユークリッド空間
...
クリフォード代数$\operatorname{Cl}_{p,q}(\mathrm K)$は、体$\mathrm K$上のベクトル空間$V$に対して定義される代数系です。ベクトル空間の次元は$p+q$で、計量$1$の基底が$p$個、計量$-1$の基底が$q$個あります。基底の2乗は計量となります。
...
&&&rem 添え字
$\operatorname{Cl}_{p,q}(\mathrm K)$は英語版Wikipediaの表記法です。[[wiki-clif]]この表記法は統一されておらず、異なる流儀があります。具体的には$p$が計量-1、$q$が計量1の基底の個数を表す場合があります。テキストによって異なるため、どのような流儀を採用しているか必ずご確認ください。
&&&
...
&&&thm 代数の同型
$$
\operatorname{Cl}_{0,1}(\mathbb R)\cong\mathbb C,\quad
...
&&&rem 3次元
3次元は$\operatorname{Cl}_{3,0}(\mathbb R)$となり、四元数と同型の$\operatorname{Cl}_{0,2}(\mathbb R)$とは異なる代数です。そのため細かい部分の符号などが四元数とは異なります。ただし、四元数のように3次元と4次元で計算方法が変わるようなことはなく、次元が変わっても使い勝手は基本的に同じです。
&&&
