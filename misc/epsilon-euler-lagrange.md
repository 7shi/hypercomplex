...
&&&rem
$\varepsilon$は任意の実数としていますが、評価するのは$\varepsilon=0$における微分係数のため、実際に使うのは$0$付近の値だけです。
&&&
...
&&&rem
ここで用いている$\partial L/\partial q$や$\partial L/\partial \dot{q}$は、$L$を独立な3引数の関数とみなした形式的な偏微分です。$\dot q = dq/dt$という関係はこの段階では使っておらず、$q$と$\dot q$を独立な引数として扱うことに矛盾は生じません。
&&&
...
&&&rem
関数$\eta$が「微小である」ことを直接定義しようとすると曖昧になりがちですが、実数$\varepsilon$が$0$に近づくことは通常の解析学で厳密に扱えます。$\varepsilon$と$\eta$を分離する意味は、この「微小性」を実数の言葉に還元し、近似ではなく通常の微分として扱えるようにする点にあります。
&&&
